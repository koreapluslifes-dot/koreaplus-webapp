/* build-og-images.cjs — generate the 14 per-language social share cards used by
   the OG/Twitter viral loop. Each is a branded 1200×630 PNG at
...
function card(lang) {
  ...
  const fs2 = lines.some(l => l.length > 18) ? 64 : 74;   // shrink for long lines
  ...
  const wordmark = `<text x="${x}" y="130" font-family="${FONTS}" font-size="40" font-weight="900" fill="#ffffff" text-anchor="${anchor}" letter-spacing="1">K·BEAUTY <tspan font-weight="400" fill="#ffd6e8" font-size="26">by KoreaPlus</tspan></text>`;
  const urlPill = `<g><rect x="${rtl ? W - 80 - 430 : 80}" y="500" width="430" height="58" rx="29" fill="rgba(255,255,255,.14)" stroke="rgba(255,255,255,.35)"/><text x="${rtl ? W - 80 - 215 : 295}" y="538" font-family="${FONTS}" font-size="26" font-weight="700" fill="#ffffff" text-anchor="middle">koreaplus-lifes.com/kbeauty</text></g>`;
  ...
  <circle cx="${rtl ? 210 : 1000}" cy="150" r="150" fill="#ffffff" opacity="0.06"/>
  <circle cx="${rtl ? 120 : 1090}" cy="470" r="220" fill="#ffffff" opacity="0.05"/>
  <text x="${rtl ? 150 : 985}" y="430" font-size="240" opacity="0.14" text-anchor="middle">✦</text>
  ${wordmark}
  <rect x="${rtl ? W - 80 - 90 : 80}" y="168" width="90" height="7" rx="3" fill="#ffd6e8"/>
  ${tagSvg}
  ${urlPill}
</svg>`;
  return svg;
...
(async () => {
  ...
  console.log(`wrote ${n} OG cards → kb/og/kb-og-<lang>.png (1200×630)`);
