...
// Standalone K-Pop channel lives at /kpop (not /guide/kpop).
function kpopLangDir(lang) {
  if (lang === 'en') return '';
  const d = (L10N[lang] && L10N[lang].dir) || lang;
  return (d.endsWith('/') ? d : d + '/');
}
function kpopPublicUrl(lang, pageSlug) {
  const s = String(pageSlug).replace(/\.html$/, '');
  return '/' + kpopLangDir(lang) + 'kpop/' + s;
}
function kpopVsPublicUrl(lang, pageSlug) {
  const s = String(pageSlug).replace(/\.html$/, '');
  return '/' + kpopLangDir(lang) + 'kpop-vs/' + s;
}
function kpopFileRel(lang, leaf) {
  return kpopLangDir(lang) + String(leaf).replace(/^\/+/, '');
}
...
const GUIDE_OG_LANGS = new Set(['en', 'ko', 'ja', 'zh', 'es', 'fr', 'de', 'pt', 'id', 'ar', 'hi', 'ru', 'vi', 'th']);
const guideOgFor = lang => `${ORIGIN}/guide/og/guide-og-${GUIDE_OG_LANGS.has(lang) ? lang : 'en'}.png`;
...
const CAT_META = {
  ...
  travel:    { label: 'Travel',          icon: '??', noun: 'destination' },
  ...
  shopping:  { label: 'Shopping',        icon: '???', noun: 'spot' },
...
const AFF_T = {
  en: { label: c => `?? Where to stay in ${c}`,        cta: c => `??? ${c} Hotels & Stays`, sub: 'Agoda ? compare best-price deals', disc: 'Affiliate link ? we may earn a small commission at no extra cost to you. It helps keep KoreaPlus free.' },
  ja: { label: () => '?? ?????????',            cta: () => '??? ?????????',   sub: 'Agoda ? ????????',        disc: '?????????? ? ???????????????????????????????????????KoreaPlus ??????????' },
  zh: { label: () => '?? ??????',                 cta: () => '??? ???????',       sub: 'Agoda ? ??????',           disc: '???? ? ?????????????????????????????? KoreaPlus ?????' },
  es: { label: () => '?? D?nde alojarte en Corea',     cta: () => '??? Hoteles y alojamiento', sub: 'Agoda ? compara las mejores ofertas', disc: 'Enlace de afiliado ? podr?amos ganar una peque?a comisi?n sin coste adicional para ti. Ayuda a mantener KoreaPlus gratis.' },
  ar: { label: () => '?? ??? ???? ?? ?????',            cta: () => '??? ????? ????? ?????? ???????', sub: 'Agoda ? ???? ???? ???????', disc: '???? ???????? ? ?? ???? ??? ????? ????? ??? ?? ????? ?????? ????. ??? ??????? ??? ????? KoreaPlus ???????.' },
  hi: { label: () => '?? ?????? ??? ???? ?????',            cta: () => '??? ?????? ?? ???? ?? ????', sub: 'Agoda ? ????? ?????? ??? ?? ????? ????', disc: '???????? ???? ? ???? ???? ????? ??? ???? ??, ???? ??? ???????? ???? ???? ???? ????? ???? KoreaPlus ???? ??? ???? ???' },
  ru: { label: () => '?? ??? ???????????? ? ?????',     cta: () => '??? ????? ? ????? ? ?????', sub: 'Agoda ? ????????? ?????? ???', disc: '??????????? ?????? ? ?? ????? ???????? ????????? ????????, ??? ??? ???? ?? ?????????. ??? ???????? KoreaPlus ?????????? ??????????.' },
  vi: { label: () => '?? ? ??u khi ??n H?n Qu?c',       cta: () => '??? Kh?ch s?n v? ch? ? t?i H?n Qu?c', sub: 'Agoda ? so s?nh gi? t?t nh?t', disc: 'Li?n k?t ti?p th? ? ch?ng t?i c? th? nh?n m?t kho?n hoa h?ng nh? m? b?n kh?ng ph?i tr? th?m chi ph? n?o. ?i?u n?y gi?p KoreaPlus lu?n mi?n ph?.' },
  th: { label: () => '?? ???????????????????',              cta: () => '??? ???????????????????????', sub: 'Agoda ? ?????????????????', disc: '????????????? ? ???????????????????????????????? ?????????????????????? ??????????? KoreaPlus ????????????' },
};
...
const KLOOK_T = {
  en: { h: '??? Popular Korea activities & travel essentials', s: 'eSIM ? airport transport ? tours ? book in seconds', disc: 'Affiliate ? we may earn a small commission at no extra cost to you.' },
  ko: { h: '??? ?? ?? ???? ? ?? ? eSIM', s: 'eSIM ? ?? ?? ? ?? ? ?? ??', disc: '?? ?? ? ?? ?? ?? ??? ???? ?? ? ???.' },
  ja: { h: '??? ?????????????eSIM???', s: 'eSIM??????????????', disc: '??????? ? ????????????????????????' },
  zh: { h: '??? ?????? ? eSIM ? ??', s: 'eSIM ? ???? ? ??? ? ????', disc: '???? ? ???????????????????' },
  es: { h: '??? Actividades y esenciales de Corea', s: 'eSIM ? transporte al aeropuerto ? tours ? reserva al instante', disc: 'Enlace de afiliado ? podr?amos ganar una peque?a comisi?n sin coste adicional.' },
  fr: { h: '??? Activit?s et essentiels en Cor?e', s: 'eSIM ? transport a?roport ? visites ? r?servez en quelques secondes', disc: 'Lien affili? ? nous pouvons percevoir une petite commission sans surco?t.' },
  de: { h: '??? Beliebte Aktivit?ten & Reise-Essentials in Korea', s: 'eSIM ? Flughafentransfer ? Touren ? sofort buchen', disc: 'Affiliate-Link ? wir erhalten ggf. eine kleine Provision ohne Mehrkosten f?r dich.' },
  pt: { h: '??? Atividades e essenciais da Coreia', s: 'eSIM ? transporte do aeroporto ? passeios ? reserve em segundos', disc: 'Link de afiliado ? podemos receber uma pequena comiss?o sem custo extra para voc?.' },
  id: { h: '??? Aktivitas & kebutuhan perjalanan di Korea', s: 'eSIM ? transportasi bandara ? tur ? pesan seketika', disc: 'Tautan afiliasi ? kami dapat memperoleh komisi kecil tanpa biaya tambahan untuk Anda.' },
  ...
  hi: { h: '??? ?????? ?? ?????? ????????? ?? ?????? ????????', s: 'eSIM ? ???????? ??????????? ? ??? ? ??? ?? ????? ??? ??? ????', disc: '???????? ???? ? ???? ???? ????? ??? ???? ??, ???? ??? ???????? ???? ???? ???? ?????' },
  ru: { h: '??? ?????????? ??????????? ? ??? ?????? ??? ??????? ? ?????', s: 'eSIM ? ???????? ?? ????????? ? ????????? ? ????? ?? ???????', disc: '??????????? ?????? ? ?? ????? ???????? ????????? ????????, ??? ??? ???? ?? ?????????.' },
  vi: { h: '??? Ho?t ??ng n?i b?t v? ?? d?ng thi?t y?u khi ?i H?n Qu?c', s: 'eSIM ? ??a ??n s?n bay ? tour ? ??t trong v?i gi?y', disc: 'Li?n k?t ti?p th? ? ch?ng t?i c? th? nh?n m?t kho?n hoa h?ng nh? m? b?n kh?ng ph?i tr? th?m chi ph? n?o.' },
  th: { h: '??? ????????????????????????????????????????????', s: 'eSIM ? ??????????????? ? ????? ? ????????????????????', disc: '????????????? ? ???????????????????????????????? ??????????????????????' }
};
...
const NEIGHBORHOODS = [
  { name: 'Hongdae', kr: '??', city: 'Seoul', emoji: '??', known: 'youth culture, indie music, street art, nightlife', do: 'Watch free street busking, browse indie fashion, club-hop after dark, hunt K-pop merch.', eat: 'Cheap student eats, themed cafes, late-night pojangmacha (street tents).', getto: 'Hongik University Station (Line 2 / AREX), Exit 9.' },
  { name: 'Gangnam', kr: '??', city: 'Seoul', emoji: '???', known: 'luxury, K-pop agencies, plastic surgery, upscale dining', do: 'See the COEX Starfield Library, K-Star Road, designer shopping, rooftop bars.', eat: 'Trendy upscale restaurants, dessert cafes, Korean fine dining.', getto: 'Gangnam Station (Line 2) or COEX via Samseong Station (Line 2).' },
  { name: 'Myeongdong', kr: '??', city: 'Seoul', emoji: '???', known: 'shopping, K-beauty, street food', do: 'Shop 50+ cosmetics stores, graze the night street-food market, visit Myeongdong Cathedral.', eat: 'Legendary street food ? egg bread, tornado potato, grilled lobster, hotteok.', getto: 'Myeongdong Station (Line 4), Exit 6.' },
  { name: 'Insadong', kr: '???', city: 'Seoul', emoji: '??', known: 'traditional crafts, teahouses, art galleries', do: 'Browse antiques and hanji paper, sip traditional tea, visit Ssamziegil mall.', eat: 'Traditional Korean tea, bibimbap, hotteok, temple-style food.', getto: 'Anguk Station (Line 3), Exit 6.' },
  { name: 'Itaewon', kr: '???', city: 'Seoul', emoji: '??', known: 'international food, nightlife, diversity', do: 'Eat global cuisine, explore antique shops, enjoy the most international nightlife in Korea.', eat: 'Halal Korean & Middle Eastern, craft beer, world cuisine.', getto: 'Itaewon Station (Line 6).' },
  { name: 'Bukchon', kr: '??', city: 'Seoul', emoji: '???', known: 'hanok village, palaces, photography', do: 'Wander 600-year-old hanok alleys between two palaces; go early for golden light and fewer crowds.', eat: 'Quiet hanok cafes, traditional sweets, tea houses.', getto: 'Anguk Station (Line 3), Exit 2.' },
];
const STAY = [
  { city: 'Seoul', kr: '??', areas: [
...
const REGIONAL_CITIES = [{ name: 'Gangneung', kr: '??' }, { name: 'Sokcho', kr: '??' }, { name: 'Suwon', kr: '??' }, { name: 'Daegu', kr: '??' }, { name: 'Daejeon', kr: '??' }, { name: 'Taean', kr: '??' }, { name: 'Tongyeong', kr: '??' }, { name: 'Pohang', kr: '??' }, { name: 'Chuncheon', kr: '??' }, { name: 'Damyang', kr: '??' }, { name: 'Jinju', kr: '??' }, { name: 'Suncheon', kr: '??' }, { name: 'Boseong', kr: '??' }, { name: 'Mokpo', kr: '??' }, { name: 'Gongju', kr: '??' }, { name: 'Korea Hiking', kr: '?? ??' }];
...
const BUSAN_LOCAL_LABEL = {
  en: '?? Busan Like a Local', ko: '?? ???? ?? ??', ja: '?? ?????????',
  zh: '?? ?????????', es: '?? Busan como un local', fr: '?? Busan comme un local',
...
function cityClusterLinks(cityName, excludeUrl) {
  ...
  const links = [];
  for (const [cn, days] of ITIN) if (cn === cityName) links.push([`itinerary/${cs}-${days}-day-itinerary.html`, `??? ${days}-Day ${cityName} Itinerary`]);
...
const L10N = {
  ja: {
    ...
    tw: { weatherH: '??????', wearH: '?? ???????', doH: '?? ?????????', evH: '?? ????????', evP: '?????????????????????????????', faqH: '? ??????', q1: m => `${m}??????????????`, q2: m => `${m}????????????????`, lead: m => `${m}????????????????????????????????????`, badge: '', ctaH: '????????????', ctaP: 'AI????????????????????', titleSuffix: '??? ? ??????????', monthEn: ['January','February','March','April','May','June','July','August','September','October','November','December'] },
    visa: {
      h1: '???? & K-ETA ?????', titleSuffix: '?2026???| KoreaPlus',
      desc: '???????????K-ETA??????????????????????????',
      lead: '??????????????????????????K-ETA??????????????????????2026??????????????',
      sections: [
        ['?? ??????', '??????????<strong>110????</strong>??????????<strong>????</strong>?30?90??????????????????????????????????????'],
        ['? K-ETA???', 'K-ETA???ESTA??????????????????????????????????????<strong>?????????????????</strong>???????????????????????????'],
        ['?? ????', '<ol class="steps"><li>????? <strong>k-eta.go.kr</strong>????????????? ? ?????????????</li><li>???<strong>72?????</strong>????</li><li>????????????????????</li><li>??? <strong>?10,000???</strong>?????????</li><li>???????? ? <strong>2?3??</strong>???????????</li></ol>'],
        ['?? ??????', '<ul class="tips"><li>?????????????????</li><li>K-ETA?????????????????????</li><li>????????????Q-CODE????</li></ul>'],
      ],
      faq: [
        ['??????????????????', '????90??????????K-ETA?????????????????? ? ???? k-eta.go.kr ??????????????'],
        ['K-ETA?????', '?10,000??????????????????????????'],
        ['?????????', '???72?????3?????????????'],
      ],
      ctaH: '??????????????', ctaP: 'AI???????????????',
  ...
  zh: {
    ...
    visa: {
      ...
      lead: '???????????????????? K-ETA???????????????????????????????2026??????',
      sections: [
        ['?? ??????', '<strong>110????/??</strong>????????????????????????????30?90??<strong>???????????????</strong>????/??????????????????????????????????????'],
        ['? ??? K-ETA?', 'K-ETA ???? ESTA???????????????????????????????<strong>???? K-ETA</strong>????????????'],
        ['?? ????', '<ol class="steps"><li>?????? <strong>k-eta.go.kr</strong>????App????????????</li><li>?????<strong>72???</strong>?????</li><li>????????????????</li><li>????? <strong>10,000??</strong>?</li><li>??????<strong>2?3??</strong>??????</li></ol>'],
        ['?? ????', '<ul class="tips"><li>??????????????</li><li>?? K-ETA ??????????</li><li>????????? Q-CODE ?????</li></ul>'],
      ],
      faq: [
        ['?????????????', '??????????????????????????????????????????????????????????????'],
  ...
  es: {
    dir: 'es', months: [
      ['enero', '-2?C a 5?C, fr?o y seco', 'Abrigo grueso, ropa t?rmica y guantes', 'Esqu?, festival de pesca en hielo de Hwacheon, iluminaciones de invierno', 'Pocas multitudes, precios m?nimos y palacios nevados'],
      ['febrero', '0?C a 7?C, A?o Nuevo Lunar', 'Varias capas de abrigo y bufanda', 'Seollal (A?o Nuevo Lunar), termas y jjimjilbang', 'Cultura tradicional con menos turistas'],
      ...
      ['abril', '10?C a 18?C, plena primavera', 'Capas ligeras y chaqueta fina', 'Cerezos en flor (Jinhae, Se?l, Gyeongju) y festivales de primavera', 'Los cerezos en flor ? Corea en su momento m?s bello'],
      ['mayo', '15?C a 23?C, c?lido y verde', 'Camiseta y una capa ligera', 'Festival de los Faroles de Loto, campos de t? de Boseong, senderismo', 'El mejor clima del a?o y paisajes verdes'],
      ['junio', '20?C a 27?C, inicio del verano y lluvias', 'Ropa de verano y paraguas', 'Playas abiertas y monta?as verdes', 'Calor agradable antes de la temporada alta'],
      ['julio', '24?C a 30?C, calor h?medo (monz?n)', 'Ropa transpirable y paraguas', 'Festival del Barro de Boryeong y noches junto al r?o Han', 'Festivales y vida nocturna pese a la lluvia'],
      ['agosto', '25?C a 33?C, el mes m?s caluroso', 'Ropa ligera, protector solar y gorra', 'Playas (Haeundae), festivales de verano, monta?as frescas', 'Plena temporada de playa y eventos veraniegos'],
      ['septiembre', '20?C a 28?C, de c?lido a fresco', 'Capas ligeras', 'Chuseok (acci?n de gracias coreana) y primer follaje', 'Clima agradable y cultura de la cosecha'],
      ['octubre', '10?C a 22?C, oto?o fresco', 'Chaqueta ligera por capas', 'Follaje oto?al (Seoraksan, Naejangsan), danza de m?scaras de Andong, Festival de Cine de Busan', 'Colores de oto?o espectaculares ? temporada ideal'],
      ['noviembre', '5?C a 15?C, oto?o tard?o', 'Abrigo y capas', '?ltimo follaje y primeras iluminaciones', 'Menos gente y paisajes dorados'],
      ['diciembre', '-3?C a 6?C, pleno invierno', 'Abrigo grueso, guantes y gorro', 'Mercados navide?os, esqu? y termas', 'Luces festivas y ambiente invernal'],
    ],
    tw: { weatherH: ': clima en Corea', wearH: '?? Qu? llevar', doH: '?? Qu? hacer', evH: '?? Eventos en curso', evP: 'El calendario de festivales de Corea se actualiza en tiempo real.', faqH: '? Preguntas frecuentes', q1: m => `?Es ${m} una buena ?poca para visitar Corea?`, q2: m => `?Qu? ropa llevar a Corea en ${m}?`, lead: m => `?Planeas viajar a Corea en ${m}? Aqu? tienes el clima, qu? llevar y qu? hacer.`, badge: '', ctaH: 'Planifica tu viaje a Corea', ctaP: 'La IA crea tu itinerario diario gratis.', titleSuffix: ' en Corea ? Clima, ropa y qu? hacer', monthEn: ['January','February','March','April','May','June','July','August','September','October','November','December'] },
    visa: {
      h1: 'Visado para Corea y K-ETA: gu?a completa', titleSuffix: ' (2026) | KoreaPlus',
      desc: '?Necesitas visado para Corea del Sur? Gu?a K-ETA en espa?ol: pa?ses exentos, c?mo solicitarlo, coste, plazos y requisitos de entrada.',
      lead: 'La mayor?a de los pa?ses hispanohablantes (Espa?a, M?xico, Chile, Argentina, Colombia, Per? y m?s) pueden entrar en Corea sin visado, pero suele requerirse la K-ETA (autorizaci?n electr?nica de viaje). Gu?a actualizada para 2026.',
      sections: [
        ['?? ?Necesito visado?', 'Los ciudadanos de <strong>m?s de 110 pa?ses</strong> ? incluida la mayor?a de Hispanoam?rica y Espa?a ? pueden permanecer 30?90 d?as sin visado por turismo (seg?n nacionalidad). Confirma siempre las condiciones de tu pa?s antes de reservar.'],
        ['? ?Qu? es la K-ETA?', 'La K-ETA es una autorizaci?n electr?nica similar al ESTA de EE.UU. Los viajeros exentos de visado suelen necesitarla antes de embarcar. Corea aplica <strong>exenciones temporales</strong> a algunos pa?ses ? consulta el portal oficial antes de viajar.'],
        ['?? C?mo solicitarla', '<ol class="steps"><li>Entra en el sitio oficial <strong>k-eta.go.kr</strong> (o la app) ? cuidado con webs no oficiales m?s caras.</li><li>Solic?tala al menos <strong>72 horas antes</strong> de la salida.</li><li>Sube tu foto, datos del pasaporte y del viaje.</li><li>Paga unos <strong>?10.000</strong> con tarjeta.</li><li>Recibe la aprobaci?n por correo ? v?lida <strong>2?3 a?os</strong> con entradas m?ltiples.</li></ol>'],
        ['?? Al llegar', '<ul class="tips"><li>Ten a mano la direcci?n del alojamiento y el billete de vuelta.</li><li>Guarda captura e impresi?n de la aprobaci?n K-ETA.</li><li>Rellena la tarjeta de llegada o Q-CODE si se solicita.</li></ul>'],
      ],
      faq: [
        ['?Los espa?oles y latinoamericanos necesitan visado para Corea?', 'La mayor?a puede entrar sin visado entre 30 y 90 d?as por turismo. Puede requerirse la K-ETA salvo exenci?n temporal ? verifica en k-eta.go.kr antes de volar.'],
        ['?Cu?nto cuesta la K-ETA?', 'Unos ?10.000 (7?8 USD), pago online con tarjeta. Usa solo el sitio oficial.'],
        ['?Cu?nto tarda la aprobaci?n?', 'De unos minutos hasta 72 horas. Solic?tala al menos 3 d?as antes de viajar.'],
      ],
...
<h2>?? 7 easy ways to cut costs</h2>
<ul class="tips">
<li>Eat where office workers eat at lunch ? set menus (??) are ?8,000?10,000 and excellent.</li>
...
<li>Don't skip the countryside: Gyeongju, Jeonju and Busan are 2?3 hours away by KTX and worth it.</li>
  ...
  {
    slug: 'is-korea-safe', emoji: '???', date: TODAY,
...
<li>Korea has excellent free WiFi (subway, cafes), so mid-tier data plans are usually plenty.</li>
  ...
  },
  {
    ...
    body: `
<p class="lead">Korea might be the world's most <strong>solo-friendly country</strong>: ultra-safe streets, a built-in culture of eating and doing things alone (??/??), and transit so good you never need a tour group.</p>
...
<ul class="tips">
<li>Look for ?? (honbap) signage or counter seating ? common in Hongdae, university areas and food courts.</li>
...
<h2>??? Perfect solo itinerary (7 days)</h2>
<ol class="steps">
...
<li>Norebang (karaoke) with new hostel friends is the universal icebreaker.</li>
    ...
    faq: [
      ...
      ['Is it weird to eat alone in Korea?', 'Not at all ? solo dining (??) is a recognized lifestyle with counter seating and single portions widely available.'],
    ],
  },
];
...
const TABBAR = {
  ...
  pt: ['In?cio', 'Explorar', 'Buscar', 'Planejar', 'Salvos'],
  id: ['Beranda', 'Jelajahi', 'Cari', 'Rencana', 'Disimpan'],
  ar: ['????????', '??????', '???', '?????', '?????????'],
  hi: ['???', '?????????', '???', '?????', '???'],
  ru: ['???????', '?????', '?????', '???????', '?????????'],
  vi: ['Trang ch?', 'Kh?m ph?', 'T?m ki?m', 'L?ch tr?nh', '?? l?u'],
  th: ['???????', '?????', '?????', '?????????', '?????????'],
};
...
const ARIA = {
  ...
  ja: { skip: '??????????', nav: '???????', top: '??????' },
  zh: { skip: '??????', nav: '??', top: '????' },
  es: { skip: 'Saltar al contenido', nav: 'Navegaci?n', top: 'Volver arriba' },
  fr: { skip: 'Aller au contenu', nav: 'Navigation', top: 'Retour en haut' },
  de: { skip: 'Zum Inhalt springen', nav: 'Navigation', top: 'Nach oben' },
  pt: { skip: 'Pular para o conte?do', nav: 'Navega??o', top: 'Voltar ao topo' },
  id: { skip: 'Lewati ke konten', nav: 'Navigasi', top: 'Kembali ke atas' },
  ar: { skip: '????? ??? ???????', nav: '??????', top: '?????? ??? ??????' },
  hi: { skip: '??????? ?? ????', nav: '????????', top: '??? ????' },
  ru: { skip: '??????? ? ??????????', nav: '?????????', top: '??????' },
  vi: { skip: 'B? qua ??n n?i dung', nav: '?i?u h??ng', top: 'L?n ??u trang' },
  th: { skip: '????????????????', nav: '????????', top: '??????????' },
};
...
function tabBar(lang) {
  ...
  return `<nav class="kp-tabbar" aria-label="Mobile navigation">
    ...
    <a href="plan.html" data-tab="plan"><span class="kp-tab-i" aria-hidden="true">???</span><span class="kp-tab-l">${T[3]}</span></a>
    <a href="bucket-list.html" data-tab="saved"><span class="kp-tab-i" aria-hidden="true">??</span><span class="kp-tab-l">${T[4]}</span></a>
  </nav>
...
function primaryNav(lang) {
  ...
  const items = [
    ...
    ['??', 'travel-basics.html', nm('basics', 'Travel Basics')],
    ['???', 'itineraries.html', nm('itineraries', 'Itineraries')],
  ...
  <div class="kp-pnav">
    ...
    <div class="kp-pnav-items">
      ...
      <a class="kp-pnav-cta" href="plan.html" data-i18n="nav.plan">??? Plan Trip</a>
    </div>
  </div>`;
}
...
const TOC_H = { en: 'On this page', ko: '??', ja: '??', zh: '??', es: 'En esta p?gina', fr: 'Sur cette page', de: 'Auf dieser Seite', pt: 'Nesta p?gina', id: 'Di halaman ini', ar: '?? ??? ??????', hi: '?? ??? ??', ru: '??????????', vi: 'Trong b?i n?y', th: '??????' };
function injectToc(body, lang) {
  ...
  const items = heads.map((h, idx) => {
    const label = h[1].replace(/<[^>]+>/g, '').replace(/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}?-??-??]/gu, '').trim();
...
const TRUST = {
  ...
  ko: { by: 'KoreaPlus ???', upd: '????', rev: '2026? ?? ?? ??' },
  ja: { by: 'KoreaPlus ?????', upd: '??', rev: '2026???????' },
  zh: { by: 'KoreaPlus ????', upd: '??', rev: '2026 ???' },
  es: { by: 'Equipo editorial de KoreaPlus', upd: 'Actualizado', rev: 'Verificado para 2026' },
  fr: { by: '?quipe ?ditoriale KoreaPlus', upd: 'Mis ? jour le', rev: 'V?rifi? pour 2026' },
  de: { by: 'KoreaPlus-Redaktion', upd: 'Aktualisiert', rev: 'Gepr?ft f?r 2026' },
  ...
  ar: { by: '???? ????? KoreaPlus', upd: '??? ?????', rev: '????? ??????? ???? 2026' },
  hi: { by: 'KoreaPlus ???????? ???', upd: '?????', rev: '2026 ?? ??? ????-????' },
  ru: { by: '???????? KoreaPlus', upd: '?????????', rev: '????? ????????? ?? 2026 ???' },
  vi: { by: 'Ban bi?n t?p KoreaPlus', upd: 'C?p nh?t', rev: '?? ki?m ch?ng cho 2026' },
  th: { by: '???????????????? KoreaPlus', upd: '???????????', rev: '?????????????????????? ?? 2026' },
};
...
function sourcesBlock(sources, { label = 'Official sources', note = '', lang = 'en' } = {}) {
  ...
  const L = { en: 'Last verified', ja: '????', zh: '????', es: '?ltima verificaci?n', ko: '?? ??', fr: 'Derni?re v?rification', de: 'Zuletzt gepr?ft', pt: '?ltima verifica??o', id: 'Terakhir diperiksa' };
  ...
  return `<aside class="seo-sources" style="margin:22px 0;padding:13px 16px;background:var(--card,rgba(255,255,255,.04));border:1px solid var(--border,rgba(255,255,255,.1));border-radius:12px;font-size:13px;line-height:1.6">`
    ...
    + `<p style="margin:7px 0 0;color:var(--text2,#9fb0c3);font-size:12px">??? ${esc(verified)}</p></aside>`;
}
// Localized city-guide key facts (ja/zh/es).
const CITY_FACTS = {
  ja: c => [`??? ???: 4?5??9?11?`, `??? ???`, `?? ??? (?)`, `?? ?? ${c === 'Seoul' ? '3?4' : '2?3'}?`, (c === 'Seoul' || c === 'Incheon') ? '?? ??????' : '?? ?????KTX/??'],
  zh: c => [`??? ??: 4?5??9?11?`, `??? ??`, `?? ?? (?)`, `?? ?? ${c === 'Seoul' ? '3?4' : '2?3'} ?`, (c === 'Seoul' || c === 'Incheon') ? '?? ????' : '?? ???? KTX/??'],
  es: c => [`??? Mejor ?poca: abr?may y sep?nov`, `??? Coreano`, `?? KRW (?)`, `?? ${c === 'Seoul' ? '3?4' : '2?3'} d?as`, (c === 'Seoul' || c === 'Incheon') ? '?? Aeropuerto de Incheon' : '?? KTX/bus desde Se?l'],
  ko: c => [`??? ???: 4~5??9~11?`, `??? ???`, `?? ? (?)`, `?? ?? ${c === 'Seoul' ? '3~4' : '2~3'}?`, (c === 'Seoul' || c === 'Incheon') ? '?? ??????' : '?? ???? KTX/??'],
  fr: c => [`??? Id?al : avr?mai & sep?nov`, `??? Cor?en`, `?? KRW (?)`, `?? ${c === 'Seoul' ? '3?4' : '2?3'} jours`, (c === 'Seoul' || c === 'Incheon') ? '?? A?roport d?Incheon' : '?? KTX/bus depuis S?oul'],
  de: c => [`??? Beste Zeit: Apr?Mai & Sep?Nov`, `??? Koreanisch`, `?? KRW (?)`, `?? ${c === 'Seoul' ? '3?4' : '2?3'} Tage`, (c === 'Seoul' || c === 'Incheon') ? '?? Flughafen Incheon' : '?? KTX/Bus ab Seoul'],
  pt: c => [`??? Melhor ?poca: abr?maio e set?nov`, `??? Coreano`, `?? KRW (?)`, `?? ${c === 'Seoul' ? '3?4' : '2?3'} dias`, (c === 'Seoul' || c === 'Incheon') ? '?? Aeroporto de Incheon' : '?? KTX/?nibus de Seul'],
  id: c => [`??? Terbaik: Apr?Mei & Sep?Nov`, `??? Bahasa Korea`, `?? KRW (?)`, `?? ${c === 'Seoul' ? '3?4' : '2?3'} hari`, (c === 'Seoul' || c === 'Incheon') ? '?? Bandara Incheon' : '?? KTX/bus dari Seoul'],
};
...
function trustBlock(lang, cdate) {
  ...
  return `<div class="seo-byline" style="font-size:12px;color:var(--text3,#8a93a0);margin:4px 0 16px;display:flex;flex-wrap:wrap;gap:6px 12px;align-items:center">` +
    `<span>?? <a href="about.html" style="color:var(--accent2,#74b9ff);text-decoration:none">${esc(t.by)}</a></span>` +
...
function placeEmoji(name) {
  ...
  const m = [
    [/palace|gyeongbok|changdeok|deoksu/, '\u{1F3EF}'], [/temple|bulguksa|jogyesa|haedong/, '\u{1F6D5}'], [/tower|n seoul/, '\u{1F5FC}'],
    [/market|sijang/, '\u{1F3EA}'], [/beach|haeundae|gwangalli/, '\u{1F3D6}'],
    [/museum|gallery/, '\u{1F3DB}'], [/mountain|\bsan\b|seoraksan|hallasan|namsan/, '\u{26F0}'],
    [/village|hanok|bukchon|gamcheon/, '\u{1F3D8}'], [/park|arboretum|garden/, '\u{1F333}'],
    [/river|han river|lake/, '\u{1F30A}'], [/island|\bdo\b|jeju|nami/, '\u{1F3DD}'],
    [/shopping|myeong|duty|mall|coex|dongdaemun|ddp/, '\u{1F6CD}'], [/bridge/, '\u{1F309}'],
    [/hot spring|spa|jjimjil/, '\u{2668}'], [/night|nightview/, '\u{1F303}'],
    [/cave/, '\u{1F573}'], [/airport|station/, '\u{2708}'], [/cafe|coffee/, '\u{2615}'],
    [/waterfall/, '\u{1F4A7}'], [/fortress|hwaseong/, '\u{1F3F0}'], [/zoo|world|theme/, '\u{1F3A2}']
  ];
...
function foodEmoji(name) {
  const n = String(name || '').toLowerCase();
  const m = [
    [/bbq|galbi|samgyeop|bulgogi|beef|pork|meat/, '\u{1F969}'],
    [/noodle|myeon|naengmyeon|ramyeon|jjajang|kalguksu/, '\u{1F35C}'],
    [/chicken|dak/, '\u{1F357}'], [/soup|jjigae|gukbap|stew|jeongol/, '\u{1F372}'],
    [/bibimbap|rice|gimbap/, '\u{1F371}'], [/seafood|sashimi|raw fish|hoe|oyster|abalone/, '\u{1F41F}'],
    [/dumpling|mandu/, '\u{1F95F}'], [/pancake|jeon|hotteok/, '\u{1F95E}'],
    [/tteokbokki|street|spicy rice/, '\u{1F361}'], [/bingsu|dessert|cake|bakery|bread|pastry/, '\u{1F370}'],
    [/soju|makgeolli|beer|alcohol|drink/, '\u{1F377}'], [/tea/, '\u{1F375}']
  ];
  for (const [re, e] of m) if (re.test(n)) return e;
  return '\u{1F35C}';
}
...
const TLDR_H = {
  en: 'Summary', ko: '??', ja: '??', zh: '??', es: 'Resumen',
  fr: 'R?sum?', de: 'Zusammenfassung', pt: 'Resumo', id: 'Ringkasan',
  it: 'Riepilogo', ru: '??????', vi: 'T?m t?t', th: '????', hi: '??????',
  ar: '??????',
};
...
function shell({ url, title, desc, keywords, schemas, hero, body, lang = 'en', alts = [], image, homeHref = 'index.html', brand = 'Korea<span>Plus</span>' }) {
  ...
  const ogImg = image || guideOgFor(lang);
  const ogH = image ? 630 : 1200;
  const isTravelHub = homeHref !== '/kpop';
...
${!image ? `<meta property="og:image:secure_url" content="${esc(ogImg)}">\n<meta property="og:image:type" content="image/png">` : ''}
...
<link rel="stylesheet" href="hub-styles.css?v=18">
<link rel="stylesheet" href="theme.css">
<link rel="stylesheet" href="seo.css?v=15">
...
${isTravelHub ? '<script defer src="modules/scroll-guard.js?v=2"></script>\n<script defer src="modules/empty-visual.js?v=4"></script>\n' : ''}<script defer src="modules/header.js"></script>
<script defer src="modules/mobile.js?v=2"></script>
...
<body${isTravelHub ? ' class="seo-hub"' : ''} data-mnav="${homeHref === '/kpop' ? 'kpop' : 'travel'}">
...
<footer class="footer" role="contentinfo" style="text-align:center;padding:40px 20px;border-top:1px solid var(--border,rgba(255,255,255,.08));margin-top:40px">
  ...
  <p style="font-size:12px;color:var(--text3,#888);margin-bottom:16px;line-height:2">
    ...
    <a href="compare.html" style="color:var(--accent2,#74b9ff)">?? Compare Cities</a> ?
...
<script defer src="modules/korea-now.js?v=4"></script>
...
<script defer src="modules/search.js?v=5"></script>
...
<div class="seo-cta">
  ...
  <p>${esc(sub)}</p>
  <div class="btns">
    <a class="primary" href="plan.html">??? Build My Free Itinerary</a>
...
const mapsHtml = q => {
  ...
  return `<div class="seo-maps">
    <a class="primary" href="https://www.google.com/maps/search/${e}" target="_blank" rel="noopener">??? Google Maps</a>
...
function buildCity(city) {
  ...
  const getThere = (city.name === 'Seoul' || city.name === 'Incheon') ? '?? Incheon Airport (AREX/metro)' : '?? KTX/bus from Seoul';

  let body = bcHtml(trail);
  const im = cityImg(city.name);
  body += heroFigure(im, `${city.name}, South Korea`);
  body += keyFactsBox([`??? Best time: Apr?May & Sep?Nov`, `??? Korean (???)`, `?? KRW (?)`, `?? Suggested: ${isHub ? '3?4' : '2?3'} days`, getThere]);
  if (g) {
  ...
  body += isRegional
    ? `<h2>??? Explore more of Korea</h2><div class="seo-linklist"><a href="explore.html">?? All destinations</a><a href="guide/things-to-do-in-seoul.html">?? Seoul</a><a href="guide/things-to-do-in-busan.html">?? Busan</a><a href="guide/korea-travel-cost-index.html">?? Cost Index 2026</a><a href="plan.html">??? AI Trip Planner</a></div>`
  ...
  body += `<h2>?? Keep planning ${esc(city.name)}</h2><div class="seo-linklist"><a href="guide/best-time-to-visit-${slug(city.name)}.html">??? Best time to visit ${esc(city.name)}</a>${TRANSPORT_ROUTES.some(r => r.to === city.name) ? `<a href="guide/seoul-to-${slug(city.name)}.html">?? Seoul ? ${esc(city.name)}</a>` : ''}<a href="guide/korea-travel-cost-index.html">?? Trip cost index</a></div>`;
  body += `<h2>?? ${esc(city.name)} on video & K-screen</h2><div class="seo-linklist"><a href="https://www.youtube.com/results?search_query=${enc(city.name + ' Korea travel')}" target="_blank" rel="noopener">?? ${esc(city.name)} travel videos</a><a href="kdrama-locations.html">?? K-drama filming locations</a><a href="kpop.html">?? K-pop in Korea</a></div>`;
...
const CITY_UI = {
  ja: { attractionsH: '?? ??????', eatH: '?? ???', gettingH: '?? ???????', tipsH: '?? ?????', faqH: '? ??????', enBtn: '???? English', planBtn: '??? ????????', moreH: '?? ?????' },
  zh: { attractionsH: '?? ????', eatH: '?? ????', gettingH: '?? ????', tipsH: '?? ????', faqH: '? ????', enBtn: '???? English', planBtn: '??? ??????', moreH: '?? ????' },
  es: { attractionsH: '?? Qu? ver', eatH: '?? Qu? comer', gettingH: '?? C?mo llegar y moverse', tipsH: '?? Consejos', faqH: '? Preguntas frecuentes', enBtn: '???? English', planBtn: '??? Crear itinerario gratis', moreH: '?? Gu?as relacionadas' },
  ko: { attractionsH: '?? ?? ?', eatH: '?? ???', gettingH: '?? ?? ????', tipsH: '?? ?? ?', faqH: '? ?? ?? ??', enBtn: '???? English', planBtn: '??? ?? ?? ???', moreH: '?? ?? ???' },
  fr: { attractionsH: '?? ? voir', eatH: '?? O? manger', gettingH: '?? Y aller et se d?placer', tipsH: '?? Conseils', faqH: '? FAQ', enBtn: '???? English', planBtn: '??? Cr?er mon itin?raire', moreH: '?? Guides li?s' },
  de: { attractionsH: '?? Sehensw?rdigkeiten', eatH: '?? Essen', gettingH: '?? Anreise & Verkehr', tipsH: '?? Tipps', faqH: '? FAQ', enBtn: '???? English', planBtn: '??? Reiseplan erstellen', moreH: '?? Verwandte Guides' },
  pt: { attractionsH: '?? O que ver', eatH: '?? O que comer', gettingH: '?? Como chegar e circular', tipsH: '?? Dicas', faqH: '? Perguntas frequentes', enBtn: '???? English', planBtn: '??? Criar meu itiner?rio', moreH: '?? Guias relacionados' },
  id: { attractionsH: '?? Tempat wisata', eatH: '?? Kuliner', gettingH: '?? Cara ke sana & transportasi', tipsH: '?? Tips', faqH: '? Pertanyaan umum', enBtn: '???? English', planBtn: '??? Buat itinerari gratis', moreH: '?? Panduan terkait' },
};
const CITY_NAME_L10N = {
  ja: { Seoul: '???', Busan: '??', Jeju: '??', Gyeongju: '??', Jeonju: '??', Incheon: '??' },
  zh: { Seoul: '??', Busan: '??', Jeju: '??', Gyeongju: '??', Jeonju: '??', Incheon: '??' },
  es: { Seoul: 'Se?l', Busan: 'Busan', Jeju: 'Jeju', Gyeongju: 'Gyeongju', Jeonju: 'Jeonju', Incheon: 'Incheon' },
  ko: { Seoul: '??', Busan: '??', Jeju: '??', Gyeongju: '??', Jeonju: '??', Incheon: '??' },
  fr: { Seoul: 'S?oul', Busan: 'Busan', Jeju: 'Jeju', Gyeongju: 'Gyeongju', Jeonju: 'Jeonju', Incheon: 'Incheon' },
...
function buildCityL10n(cityName, lang) {
  ...
  const vH = { ja: '?? ???K-?????', zh: '?? ??? K-??', es: '?? V?deos y contenido K' }[lang] || '?? Video';
  const vL = { ja: '?? ????', zh: '?? ????', es: '?? V?deos de viaje' }[lang] || '?? Videos';
  ...
  const hero = `<header class="seo-hero"><span class="emoji">??</span><h1>${esc(g.h1)}</h1><div class="kr">${esc((CITY_NAME_L10N[lang] || {})[cityName] || cityName)}</div><div class="meta"><span class="seo-badge">${g.attractions.length} ${lang === 'ja' ? '????' : lang === 'zh' ? '??' : 'lugares'}</span></div></header>`;
...
function buildItinerary(city, days, pool) {
  ...
  body += keyFactsBox([`?? ${days}-day plan`, `?? Base: ${esc(city.name)}`, `??? Best: Apr?May & Sep?Nov`, `?? First-timer friendly`]);
  ...
  for (let d = 1; d <= days; d++) {
    ...
    if (morning) body += slotHtml('??', 'Morning', morning);
    if (lunch) body += slotHtml('??', 'Lunch', lunch);
    if (afternoon && afternoon.slug !== (morning && morning.slug)) body += slotHtml('???', 'Afternoon', afternoon);
  ...
  body += `<h2>??? More Korea itineraries</h2><div class="seo-linklist">${themeRel}</div>`;
  ...
  const hero = `<header class="seo-hero"><span class="emoji">???</span><h1>${esc(h1)}</h1><div class="meta"><span class="seo-badge">${days} days</span><span class="seo-badge region">${esc(city.name)}</span></div></header>`;
...
// ??????????????????????????????????????????????????????????????????
const MONTHS = [
  ['January', '??', '-2?C to 5?C, cold & dry', 'Heavy coat, thermals, gloves', 'Ski resorts, ice fishing (Hwacheon Sancheoneo), winter illuminations', 'Few crowds, lowest prices, snowy palaces'],
  ['February', '???', '0?C to 7?C, cold, Lunar New Year', 'Warm layers, scarf', 'Seollal (Lunar New Year), late-winter snow, hot spring spas', 'Cultural festivals, quieter sights'],
  ...
  ['June', '??', '20?C to 27?C, early summer/rain start', 'Summer clothes, umbrella', 'Beaches open, Boryeong Mud Festival prep, green mountains', 'Warm, fewer crowds before peak summer'],
  ['July', '???', '24?C to 30?C, hot & humid (monsoon)', 'Light breathable clothes, umbrella', 'Boryeong Mud Festival, water parks, Han River nights', 'Festivals & nightlife despite the rain'],
  ['August', '???', '25?C to 33?C, hottest & humid', 'Light clothes, sunscreen, hat', 'Beaches (Haeundae), summer festivals, mountain escapes', 'Peak beach season, vibrant summer events'],
...
function buildMonth(m, idx) {
  ...
  body += `<h2>??? ${esc(name)} Weather in Korea</h2><p><strong>Typical temperatures:</strong> ${esc(weather)}.</p>`;
  ...
  const hero = `<header class="seo-hero"><span class="emoji">${icon}</span><h1>${esc('Korea in ' + name)}</h1><div class="meta"><span class="seo-badge">${esc(weather.split(',')[0])}</span><a class="seo-badge" href="ja/korea-in-${sslug}.html">???? ???</a><a class="seo-badge" href="zh/korea-in-${sslug}.html">???? ??</a><a class="seo-badge" href="es/korea-in-${sslug}.html">???? Espa?ol</a></div></header>`;
  const article = { '@context': 'https://schema.org', '@type': 'Article', headline: h1, description: desc, datePublished: TODAY, dateModified: TODAY, author: { '@type': 'Organization', name: 'KoreaPlus' }, publisher: { '@type': 'Organization', name: 'KoreaPlus-Lifes', logo: { '@type': 'ImageObject', url: ORIGIN + '/guide/icons/kplus.svg' } }, image: ORIGIN + '/guide/og-image.jpg', mainEntityOfPage: ORIGIN + url };
...
function buildMonthL10n(idx, lang) {
  ...
  const h1 = ({ ja: `${nameL}?????`, zh: `${nameL}??????`, es: `Corea en ${nameL}`,
    ko: `${nameL} ?? ??`, fr: `La Cor?e en ${nameL}`, de: `Korea im ${nameL}`, pt: `Coreia em ${nameL}`, id: `Korea di bulan ${nameL}` }[lang]) || `Corea en ${nameL}`;
  const title = lang === 'es' ? `Corea en ${nameL}: clima, qu? llevar y qu? hacer | KoreaPlus` : `${nameL}${tw.titleSuffix} | KoreaPlus`;
  ...
  body += `<h2>??? ${esc(nameL)}${esc(tw.weatherH)}</h2><p>${esc(weather)}${dot}</p>`;
  ...
  const moreH = { ja: '?? ???????????', zh: '?? ????????', es: '?? Corea mes a mes',
    ko: '?? ?? ? ?? ?? ???', fr: '?? La Cor?e mois par mois', de: '?? Korea Monat f?r Monat', pt: '?? Coreia m?s a m?s', id: '?? Korea bulan demi bulan' }[lang] || '?? Korea month by month';
  ...
  body += `<div class="seo-cta"><h2>${esc(tw.ctaH)}</h2><p>${esc(tw.ctaP)}</p><div class="btns"><a class="primary" href="plan.html">??? AI Trip Planner</a><a class="ghost" href="${enUrl.replace(BASEP, '')}">???? English version</a></div></div>`;
...
function buildVisaL10n(lang) {
  ...
  const bestH = { ja: '?? ??????????', zh: '?? ????????', es: '?? ?Cu?ndo viajar a Corea?',
    ko: '?? ??? ?? ??', fr: '?? Quand partir en Cor?e ?', de: '?? Wann nach Korea reisen?', pt: '?? Quando viajar para a Coreia?', id: '?? Kapan ke Korea?' }[lang] || '?? Best time to visit';
  ...
  const _vsLabel = { ja: '?????', zh: '????', es: 'Fuentes oficiales', ko: '?? ??', fr: 'Sources officielles', de: 'Offizielle Quellen', pt: 'Fontes oficiais', id: 'Sumber resmi' }[lang] || 'Official sources';
  body += sourcesBlock([
  ...
  body += `<div class="seo-cta"><h2>${esc(V.ctaH)}</h2><p>${esc(V.ctaP)}</p><div class="btns"><a class="primary" href="plan.html">??? AI Trip Planner</a><a class="ghost" href="${enUrl.replace(BASEP, '')}">???? English version</a></div></div>`;
...
const BLOG_UI = {
  ja: { faqH: '? ??????', moreH: '?? ????????', ctaH: '????????????', ctaP: 'AI?30??????????????', planBtn: '??? ????????', enBtn: '???? English', badge: '?????' },
  zh: { faqH: '? ????', moreH: '?? ??????', ctaH: '??????????', ctaP: 'AI 30??????????', planBtn: '??? ??????', enBtn: '???? English', badge: '????' },
  es: { faqH: '? Preguntas frecuentes', moreH: '?? M?s del blog', ctaH: '?Listo para planear tu viaje a Corea?', ctaP: 'Crea un itinerario diario gratis con IA.', planBtn: '??? Crear itinerario gratis', enBtn: '???? English', badge: 'Consejos de viaje' },
  ko: { faqH: '? ?? ?? ??', moreH: '?? ??? ?? ?', ctaH: '?? ??? ??? ???', ctaP: 'AI? 30? ?? ??? ??? ? ???.', planBtn: '??? ??? ?? ???', enBtn: '???? English', badge: '?? ?' },
  fr: { faqH: '? Questions fr?quentes', moreH: '?? Plus d\'articles du blog', ctaH: 'Pr?t ? planifier ton voyage en Cor?e ?', ctaP: 'Cr?e gratuitement un itin?raire jour par jour avec l\'IA.', planBtn: '??? Cr?er mon itin?raire gratuit', enBtn: '???? English', badge: 'Conseils de voyage' },
  de: { faqH: '? H?ufige Fragen', moreH: '?? Mehr aus dem Blog', ctaH: 'Bereit, deine Korea-Reise zu planen?', ctaP: 'Erstelle mit KI gratis einen Tagesplan.', planBtn: '??? Gratis Reiseplan erstellen', enBtn: '???? English', badge: 'Reisetipps' },
  pt: { faqH: '? Perguntas frequentes', moreH: '?? Mais do blog', ctaH: 'Pronto para planejar sua viagem ? Coreia?', ctaP: 'Crie um roteiro di?rio gr?tis com IA.', planBtn: '??? Criar roteiro gr?tis', enBtn: '???? English', badge: 'Dicas de viagem' },
  id: { faqH: '? Pertanyaan umum', moreH: '?? Artikel blog lainnya', ctaH: 'Siap merencanakan liburan ke Korea?', ctaP: 'Buat itinerary harian gratis dengan AI.', planBtn: '??? Buat itinerary gratis', enBtn: '???? English', badge: 'Tips perjalanan' },
};
...
const SEASON_ROWS = [
  ...
  ['???', 'Summer (Jun?Aug)', '24?33?C, hot, humid, monsoon in Jul', 'Beaches, water festivals and long days ? lively but rainy and crowded mid-summer.'],
  ['??', 'Autumn (Sep?Nov)', '10?22?C, crisp and clear', 'Spectacular foliage, harvest food and blue skies ? the other peak season.'],
  ['??', 'Winter (Dec?Feb)', '-5?6?C, cold and dry', 'Snowy palaces, skiing, hot street food and the lowest prices and crowds.'],
];
function buildBestTime(city) {
  ...
  body += keyFactsBox([`?? Best: ${esc(bt.peak.split(' and ')[0])}`, `?? Cheapest: winter (Dec?Feb)`, `?? Most crowded: cherry-blossom week & Chuseok`, `??? Worst weather: monsoon (Jul)`]);
  ...
  const hero = `<header class="seo-hero"><span class="emoji">???</span><h1>${esc(h1)}</h1><div class="kr" lang="ko">${esc(city.kr)}</div><div class="meta"><span class="seo-badge">Month-by-month</span><span class="seo-badge">2026</span></div></header>`;
...
const TRANSPORT_ROUTES = [
  { to: 'Busan', kr: '??', ktx: '2h 20m', ktxKrw: '?59,800', bus: '4h 20m', busKrw: '?28,000', flight: '~1h', note: 'The KTX from Seoul Station to Busan Station is the clear winner ? fast, frequent and city-center to city-center.' },
  { to: 'Jeju', kr: '??', ktx: null, bus: null, flight: '~1h 5m', flightKrw: '?40,000?90,000', note: 'Jeju is an island ? fly. Gimpo?Jeju is one of the world\'s busiest air routes, with flights every few minutes from Seoul (Gimpo).' },
  { to: 'Gyeongju', kr: '??', ktx: '2h (to Singyeongju)', ktxKrw: '?49,000', bus: '3h 40m', busKrw: '?24,000', note: 'Take the KTX to Singyeongju Station, then a short bus or taxi into town; express buses run direct to Gyeongju terminal.' },
  { to: 'Jeonju', kr: '??', ktx: '1h 40m (KTX) / 2h 20m (regular)', ktxKrw: '?34,000', bus: '2h 45m', busKrw: '?16,000', note: 'KTX/SRT to Jeonju Station is fastest; express buses from Seoul are cheaper and drop you central.' },
  { to: 'Incheon', kr: '??', ktx: null, bus: null, subway: '~1h (Line 1 / AREX)', subwayKrw: '?1,500?4,750', note: 'Incheon is part of greater Seoul ? just take Subway Line 1 to Incheon Station (Chinatown) or AREX toward the airport.' },
];
function buildTransport(r) {
  ...
  if (r.flight) facts.push(`?? Flight: ${r.flight}`);
  ...
  if (r.flight) rows.push(['?? Flight', r.flight, r.flightKrw || '?']);
  ...
  body += `<h2>?? Plan your trip</h2><div class="seo-linklist"><a href="guide/things-to-do-in-${ts}.html">?? Things to Do in ${esc(r.to)}</a><a href="guide/best-time-to-visit-${ts}.html">??? Best Time to Visit ${esc(r.to)}</a><a href="explore.html">?? All Korea guides</a><a href="guide/korea-travel-cost-index.html">?? Cost Index</a></div>`;
  const qa = [
...
const COST_DATA = {
  ...
  items: [['??', 0, '?3,000?5,000', '$2.5?4'], ['??', 1, '?9,000?13,000', '$7?10'], ['??', 2, '?15,000?25,000', '$11?19'], ['?', 3, '?4,500?6,000', '$3.5?4.5'], ['??', 4, '?1,400?1,500', '$1.1'], ['??', 5, '?59,800', '$44'], ['??', 6, '?3,000', '$2.2'], ['??', 7, '?90,000', '$66'], ['???', 8, '?25,000', '$18'], ['??', 9, '?5,000', '$3.7']],
  cities: [0, 1, 2, 3],
};
const COST_L10N = {
  ...
  ja: { title: '???? ???? 2026 ? 1?????????? | KoreaPlus', h1: '???? ???? 2026', desc: '2026??????????????1?$55?250????????????????????????????KoreaPlus??????', lead: '2026???????????????????????????????1????????????????????????????', budgetH: '?? ????????1???', perDay: '/?', itemsH: '?? ??????2026??', th: ['??', '?????', '???'], cityH: '?? ????????', methodH: '?? ????', method: '???????????????????????????????????????????????????????????????????????????????????????????????????????', faqH: '? ??????', styleN: { backpacker: '???????', midrange: '??', comfort: '???' }, itemN: ['????????????', '???????????', '???1??', '????????', '??????', 'KTX ??????', '???????????', '3?????/?', '??????', '??????'], cityN: [['???', '?? ? ?????????'], ['??', '??????5?10%??'], ['??', '???+????????'], ['?????', '????????'] ], faq: [['???1????????????', '????????????$500?600????$800?1,000?????7??$1,700????????'], ['?????????????', '??????????????10?25%??????????????'], ['??????????????', '????????????????????9,000?13,000??????????????????']] },
  zh: { title: '???????? 2026 ? ?????? | KoreaPlus', h1: '???????? 2026', desc: '2026 ???????????? $55?250 ?????????????????????????KoreaPlus ?????', lead: '2026 ?????????????????????????????????????????????????????', budgetH: '?? ??????????', perDay: '/?', itemsH: '?? ???????2026?', th: ['??', '??', '??'], cityH: '?? ???????', methodH: '?? ????', method: '????????????????????????????????????????????????????????????????????????', faqH: '? ????', styleN: { backpacker: '???', midrange: '??', comfort: '???' }, itemN: ['??????????', '????????', '????????', '?????', '??/??', 'KTX ?????', '???????????', '3 ???/?', '????', '??????'], cityN: [['??', '??????????'], ['??', '?????? 5?10%'], ['??', '????+??'], ['??/??', '?????????']], faq: [['????????????', '???????? $500?600??? $800?1,000???? 7 ? $1,700 ???'], ['?????????', '?????????????? 10?25%????????'], ['???????????', '?????????????????9,000?13,000 ?????????']] },
  es: { title: '?ndice de costes de viaje a Corea 2026 | KoreaPlus', h1: '?ndice de costes de viaje a Corea 2026', desc: '?Cu?nto cuesta un viaje a Corea en 2026? Presupuestos diarios reales ($55?250/d?a) y precios detallados de comida, transporte, hoteles y atracciones. Datos propios de KoreaPlus.', lead: '?Cu?nto cuesta Corea en 2026? Con precios actuales sobre el terreno, este es el presupuesto diario real por estilo de viaje y lo que cuesta cada cosa ? nuestro ?ndice propio, actualizado con frecuencia.', budgetH: '?? Presupuesto diario por estilo', perDay: '/d?a', itemsH: '?? Lo que cuestan las cosas (2026)', th: ['Concepto', 'Won', 'USD'], cityH: '?? C?mo var?a por ciudad', methodH: '?? C?mo lo calculamos', method: 'Las cifras son medianas de precios de men?s, billetes de transporte p?blico, entradas oficiales y tarifas t?picas de hotel en Corea, convertidas a tipo de cambio en vivo y redondeadas. Actualizamos el ?ndice con regularidad; la fecha de arriba indica la ?ltima actualizaci?n. No incluye vuelos.', faqH: '? Preguntas frecuentes', styleN: { backpacker: 'Mochilero', midrange: 'Gama media', comfort: 'C?modo' }, itemN: ['Aperitivo callejero (tteokbokki)', 'Comida sencilla (bibimbap)', 'BBQ coreana (por persona)', 'Caf?', 'Metro / autob?s', 'KTX Se?l?Busan', 'Entrada a palacio (gratis con hanbok)', 'Hotel 3? / noche', 'Litera en albergue', 'Soju (restaurante)'], cityN: [['Se?l', 'Referencia ? el mayor rango de precios'], ['Busan', 'Un 5?10% m?s barato que Se?l'], ['Jeju', 'Suma vuelos + coche de alquiler'], ['Gyeongju / Jeonju', 'La mejor relaci?n calidad-precio']], faq: [['?Cu?nto cuesta una semana en Corea?', 'Sin vuelos, unos $500?600 econ?mico, $800?1.000 gama media y $1.700+ c?modo en 7 d?as.'], ['?Es Corea m?s barata que Jap?n?', 'S?: alojamiento y comida cuestan un 10?25% menos que opciones comparables en Jap?n, y el transporte mucho menos.'], ['?Cu?l es el mayor gasto en Corea?', 'El alojamiento y luego el transporte entre ciudades. La comida es la ganga: comidas excelentes por 9.000?13.000 ?.']] },
};
...
function buildCostIndex(lang) {
  ...
  const _srcLabel = { en: 'Sources & methodology', ja: '???????', zh: '?????', es: 'Fuentes y metodolog?a' };
  const _srcNote = { en: 'Prices are our own field estimates cross-checked against the official references below; exchange rates fluctuate daily.', ja: '?????????????????????????????????????????', zh: '???????????????????????????', es: 'Los precios son estimaciones propias contrastadas con las referencias oficiales; el tipo de cambio var?a a diario.' };
  body += sourcesBlock([
  ...
  body += (lang === 'en') ? ctaHtml('Plan within your budget', 'Our free AI planner builds a Korea trip to match your daily budget.')
    : `<div class="seo-cta"><h2>${esc(C.h1)}</h2><div class="btns"><a class="primary" href="plan.html">??? AI Trip Planner</a><a class="ghost" href="${enUrl.replace(BASEP, '')}">???? English</a></div></div>`;
...
const FILTER_PH = { en: 'Filter guides?', ko: '??????', ja: '?????', zh: '???', es: 'Filtrar?', fr: 'Filtrer?', de: 'Filtern?', pt: 'Filtrar?', id: 'Saring?', ar: '????? ???????', hi: '???? ??????', ru: '?????? ?????', vi: 'L?c c?m nang?', th: '????????????' };
...
function buildBrowseHubsL10n(urls, lang) {
  ...
  mk('destinations.html', 'destinations', '??', [
    ...
    { title: T.hubs.destinations.sec.besttime, links: allCities.map(c => [`??? ${(CITY_NAME_L10N[lang] || {})[c.name] || c.name}`, `guide/best-time-to-visit-${slug(c.name)}.html`]) },
  ...
  mk('experiences.html', 'experiences', '??', [
    ...
    { title: T.hubs.experiences.sec.culture, links: [['?? Festivals', 'festivals.html'], ['?? Seasons', 'seasons.html'], ['??? Culture', 'culture.html'], ['?? Temples', 'temples.html'], ['?? Night views', 'nightviews.html'], ['?? K-Drama', 'kdrama-locations.html'], ['?? Korea hiking', loc('guide/things-to-do-in-korea-hiking.html', `${dir}/guide/things-to-do-in-korea-hiking.html`)]] },
  ...
  mk('when-to-visit.html', 'whentovisit', '??', [
    ...
    { title: T.hubs.whentovisit.sec.besttime, links: allCities.map(c => [`??? ${(CITY_NAME_L10N[lang] || {})[c.name] || c.name}`, `guide/best-time-to-visit-${slug(c.name)}.html`]) },
  ...
  mk('travel-basics.html', 'basics', '??', [
    ...
    { title: T.hubs.basics.sec.money, links: [['what-currency-does-korea-use', '??'], ['do-i-need-cash-in-korea', '??'], ['what-apps-do-i-need-for-korea', '??'], ['is-wifi-free-in-korea', '??'], ['can-you-use-google-maps-in-korea', '???'], ['what-power-plug-does-korea-use', '??']].map(([s, e]) => [faqLabel({ slug: s, emoji: e, q: s.replace(/-/g, ' ') }), loc(`faq/${s}.html`, `${dir}/faq/${s}.html`)]) },
    { title: T.hubs.basics.sec.lang, links: [['??? Phrases', 'phrases.html'], ['?? Etiquette', 'etiquette.html'], ['?? Menu', 'menu-translator.html']] },
  ]);

  mk('itineraries.html', 'itineraries', '???', [
    { title: T.hubs.itineraries.sec.all, links: (urls.itineraries || []).map(u => { const rel = u.replace(BASEP, ''); return [`??? ${esc(cap(u))}`, loc(rel, `${dir}/${rel}`)]; }) },
    { title: T.hubs.itineraries.sec.own, links: [['??? AI Trip Planner', 'plan.html'], ['?? Quiz', 'quiz.html'], ['?? Compare', 'compare.html']] },
  ]);

  mk('tools.html', 'tools', '??', [
    { title: T.hubs.tools.sec.plan, links: [['??? AI Trip Planner', 'plan.html'], ['?? City Quiz', 'quiz.html'], ['?? Compare Cities', 'compare.html'], ['? Bucket List', 'bucket-list.html']] },
    { title: T.hubs.tools.sec.utils, links: [['?? Currency', 'currency.html'], ['?? Menu', 'menu-translator.html'], ['??? Phrases', 'phrases.html'], ['?? Subway', 'subway.html']] },
...
function buildBrowseHubs(urls) {
  ...
  reg({ slug: 'destinations.html', h1: 'Destinations', emoji: '??', tagline: 'All 24 cities & regions ? guides, food, best time, where to stay', count: allCities.length, build: {
    ...
    sections: [
      { title: '??? Major cities', links: CITIES.map(c => [`?? ${esc(c.name)}`, `guide/things-to-do-in-${slug(c.name)}.html`]) },
      { title: '??? More destinations', links: REGIONAL_CITIES.filter(c => c.name !== 'Korea Hiking').map(c => [`?? ${esc(c.name)}`, `guide/things-to-do-in-${slug(c.name)}.html`]) },
      { title: '?? Best food by city', links: CITIES.map(c => [`?? ${esc(c.name)} food`, `guide/best-food-in-${slug(c.name)}.html`]) },
      { title: '??? Best time to visit by city', links: allCities.map(c => [`??? ${esc(c.name)}`, `guide/best-time-to-visit-${slug(c.name)}.html`]) },
      { title: '?? Where to stay', links: (urls.stays || []).map(u => [`?? ${esc(cap(u))}`, u.replace(BASEP, '')]) },
      { title: '??? Seoul neighborhoods', links: NEIGHBORHOODS.map(n => [`${n.emoji} ${esc(n.name)}`, `guide/${slug(n.name)}-${slug(n.city)}-guide.html`]) },
  ...
  reg({ slug: 'experiences.html', h1: 'Things to Do', emoji: '??', tagline: 'Browse by theme ? palaces, food, nature, nightlife & more', count: ALL.length, build: {
    ...
    sections: [
      ...
      { title: '?? Culture & seasonal', links: [['?? Festivals & events', 'festivals.html'], ['?? Seasons & bloom forecast', 'seasons.html'], ['??? Korean culture', 'culture.html'], ['?? Temples', 'temples.html'], ['?? Night views', 'nightviews.html'], ['?? K-Drama locations', 'kdrama-locations.html'], ['?? Korea hiking', 'guide/things-to-do-in-korea-hiking.html']] },
      { title: '?? Compare & decide', links: COMPARES.map(c => [`${c.emoji} ${esc(c.h1.split(':')[0])}`, `guide/${c.slug}.html`]) },
  ...
  reg({ slug: 'when-to-visit.html', h1: 'When to Visit', emoji: '??', tagline: 'Season & month-by-month guides + bloom forecasts', count: SEASONS4.length + MONTHS.length, build: {
    ...
    sections: [
      ...
      { title: '??? Best time by city', links: allCities.map(c => [`??? ${esc(c.name)}`, `guide/best-time-to-visit-${slug(c.name)}.html`]) },
  ...
  reg({ slug: 'travel-basics.html', h1: 'Travel Basics', emoji: '??', tagline: 'Visa, budget, transport, money & where to stay', count: 12, build: {
    slug: 'travel-basics.html', h1: 'Korea Travel Basics', emoji: '??',
    ...
    sections: [
      ...
      { title: '?? Where to stay', links: (urls.stays || []).map(u => [`?? ${esc(cap(u))}`, u.replace(BASEP, '')]) },
      { title: '?? Money, apps & connectivity', links: [['?? What currency does Korea use?', 'faq/what-currency-does-korea-use.html'], ['?? Do I need cash in Korea?', 'faq/do-i-need-cash-in-korea.html'], ['?? What apps do I need?', 'faq/what-apps-do-i-need-for-korea.html'], ['?? Is Wi-Fi free in Korea?', 'faq/is-wifi-free-in-korea.html'], ['??? Does Google Maps work?', 'faq/can-you-use-google-maps-in-korea.html'], ['?? What power plug?', 'faq/what-power-plug-does-korea-use.html']] },
      { title: '??? Language & etiquette', links: [['??? Survival Korean phrases', 'phrases.html'], ['?? Korean etiquette', 'etiquette.html'], ['?? Menu translator', 'menu-translator.html']] },
    ],
  } });

  reg({ slug: 'itineraries.html', h1: 'Itineraries', emoji: '???', tagline: 'Ready-made day-by-day trip plans you can customize', count: (urls.itineraries || []).length, build: {
    slug: 'itineraries.html', h1: 'Korea Itineraries', emoji: '???',
    ...
    sections: [
      { title: '??? All itineraries', links: (urls.itineraries || []).map(u => [`??? ${esc(cap(u))}`, u.replace(BASEP, '')]) },
      { title: '?? Build your own', links: [['??? Free AI Trip Planner', 'plan.html'], ['?? Which city should you visit? (Quiz)', 'quiz.html'], ['?? Compare cities', 'compare.html']] },
    ],
  } });
  ...
  reg({ slug: 'tools.html', h1: 'Free Tools', emoji: '??', tagline: 'Quiz, comparisons, bucket list, currency & live trends', count: 11, build: {
    ...
    badge: 'Interactive',
    sections: [
      { title: '?? Plan & decide', links: [['??? AI Trip Planner', 'plan.html'], ['?? City Quiz', 'quiz.html'], ['?? Compare Cities', 'compare.html'], ['? Bucket List', 'bucket-list.html']] },
      { title: '?? Handy utilities', links: [['?? Currency Converter', 'currency.html'], ['?? Menu Translator', 'menu-translator.html'], ['??? Phrasebook', 'phrases.html'], ['?? Subway', 'subway.html']] },
...
function buildExplore(urls) {
  ...
  body += section('?? Travel Basics', [`<a href="guide/korea-visa-k-eta-guide.html">?? Visa & K-ETA Guide</a>`, `<a href="guide/korea-travel-cost-index.html">?? Korea Travel Cost Index 2026</a>`, ...(urls.stays || []).map(u => `<a href="${u.replace(BASEP, '')}">?? ${esc(u.split('/').pop().replace(/-/g, ' ').replace('.html', '').replace(/\b\w/g, m => m.toUpperCase()))}</a>`)]);
  body += section('?? Fun &amp; Interactive', [`<a href="quiz.html">?? Which Korean City Should You Visit? (Quiz)</a>`, `<a href="bucket-list.html">? Korea Travel Bucket List (40 things)</a>`, `<a href="compare.html">?? Compare Korean Cities (Seoul vs Busan vs Jeju)</a>`, `<a href="trending.html">?? Trending in Korea Right Now</a>`, `<a href="plan.html">??? AI Trip Planner</a>`, `<a href="currency.html">?? Currency Converter</a>`, `<a href="embed.html">?? Free Korea Widgets (embed on your site)</a>`]);
  ...
  body += section('?? ??? ? ??? ? Multiling?e ? Mehrsprachig', LOCALES.flatMap(l => [
  ...
  if (urls.neighborhoods && urls.neighborhoods.length) body += section('??? Seoul Neighborhoods', NEIGHBORHOODS.map(n => `<a href="guide/${slug(n.name)}-${slug(n.city)}-guide.html">${n.emoji} ${esc(n.name)}</a>`));
  body += section('??? Itineraries', urls.itineraries.map(u => `<a href="${u.replace(BASEP, '')}">??? ${esc(u.split('/').pop().replace(/-/g, ' ').replace('.html', '').replace(/\b\w/g, m => m.toUpperCase()))}</a>`));
  ...
  body += section('??? Best Time to Visit', [...CITIES, ...REGIONAL_CITIES].map(c => `<a href="guide/best-time-to-visit-${slug(c.name)}.html">??? Best time: ${esc(c.name)}</a>`));
  ...
  body += section('?? Comparisons', COMPARES.map(c => `<a href="guide/${c.slug}.html">${c.emoji} ${esc(c.h1.split(':')[0])}</a>`));
...
function buildVisa() {
  ...
  body += ctaHtml('Ready to plan your Korea trip?', 'Build a free day-by-day itinerary with AI in seconds.');
  const hero = `<header class="seo-hero"><span class="emoji">??</span><h1>${esc(h1)}</h1><div class="meta"><span class="seo-badge">Updated 2026</span><a class="seo-badge" href="ja/korea-visa-k-eta-guide.html">???? ???</a><a class="seo-badge" href="zh/korea-visa-k-eta-guide.html">???? ??</a><a class="seo-badge" href="es/korea-visa-k-eta-guide.html">???? Espa?ol</a></div></header>`;
  const article = { '@context': 'https://schema.org', '@type': 'Article', headline: h1, description: desc, datePublished: TODAY, dateModified: TODAY, author: { '@type': 'Organization', name: 'KoreaPlus' }, publisher: { '@type': 'Organization', name: 'KoreaPlus-Lifes', logo: { '@type': 'ImageObject', url: ORIGIN + '/guide/icons/kplus.svg' } }, image: ORIGIN + '/guide/og-image.jpg', mainEntityOfPage: ORIGIN + url };
...
const FAQS = [
  ...
  { slug: 'do-they-speak-english-in-korea', emoji: '???', q: 'Do they speak English in Korea?',
    ...
    body: '<p>Hotels, airports and major attractions all operate in English. At local restaurants, pointing at the menu plus a translation app works perfectly. Learning two phrases ? ????? (hello) and ????? (thank you) ? earns instant goodwill.</p>',
  ...
  { slug: 'do-you-tip-in-korea', emoji: '??', q: 'Do you tip in Korea?',
    ...
    body: '<p>Service charges at high-end hotels are built into the bill. The best way to show appreciation is a sincere ????? (gamsahamnida ? thank you).</p>',
  ...
  { slug: 'can-you-use-google-maps-in-korea', emoji: '???', q: 'Does Google Maps work in Korea?',
  ...
  { slug: 'what-is-closed-on-mondays-in-korea', emoji: '???', q: 'What is closed on Mondays in Korea?',
...
const FAQ_L10N = {
  ja: {
    ui: { titleSuffix: '?2026?| KoreaPlus', badge: '??????', moreH: '? ??????', ctaH: '????????????', ctaP: 'AI?30???????????????????', planBtn: '??? ????????', enBtn: '???? English' },
    items: {
      'how-to-get-from-incheon-airport-to-seoul': ['?????????????????????', 'AREX???????????????????????43??11,000???????????1???T-money???4,750?????????????????????????????????55,000?75,000??????', '<p>AREX????????????????????????????????????????????????????17,000?18,000????????????????????????????N6001/N6703?????????Kakao T??????????????</p>'],
      'best-time-to-visit-korea': ['???????????????????', '4?5??9?11???????????????????????????????????????7?8?????????????????????????????????????', '<p>??3????????4???????????????10????11??????????????7?????????????????????</p>'],
      'do-they-speak-english-in-korea': ['?????????????', '??????????????????????????????????????????????????????????????????????????????????Papago???????????', '<p>?????????????????????????????????????????????????????????????????????????????????????????????????</p>'],
      'do-you-tip-in-korea': ['??????????????', '?????????????????????????????????????????????????????????????????????????????????', '<p>????????????????????????????????????????????????????????????????</p>'],
      'can-you-drink-tap-water-in-korea': ['?????????????', '????????????????????????WHO?????????????????????????????????????????????????????????1,000??????', '<p>??????????????????????????????????????????????????????????????</p>'],
      'what-power-plug-does-korea-use': ['?????????????????', '???220V?60Hz???????????2????C????F???????????????????????????????????????????????????220V????????????', '<p>?????100?240V?????????????????????OK???????????2,000?5,000?????????</p>'],
      'is-uber-available-in-korea': ['???Uber???????', 'Uber?????????????UT?????????????????????Kakao T???????Kakao T?????????????????????????????????????????', '<p>???????????????????????4,800?????????????????????????????????????????</p>'],
      'can-you-use-google-maps-in-korea': ['???Google??????????', '???????????????????Google???????????????????????????????????Naver Map???KakaoMap??????????????????????????????????', '<p>????Naver Map?KakaoMap??????????????????????????????????????????????????????????????????</p>'],
      'what-currency-does-korea-use': ['???????????', '??????????KRW??????????1,000????0.7?0.8?????????????????????????????5??10????????????????', '<p>???????????????????????????????????????????????Global ATM?????????</p>'],
      'do-i-need-cash-in-korea': ['????????????', '???????????????????????????????????????????????????????T-money???????5??10?????????????????', '<p>??????????????????????????????????????????????????????????WOWPASS?T-money?????????????????</p>'],
      'what-apps-do-i-need-for-korea': ['?????????????', '???4??Naver Map???KakaoMap??? ? Google????????????Papago???????????Kakao T???????Korail Talk?KTX?????????????????', '<p>?????????KakaoMap????????????????????????????????????????????AI??????</p>'],
      'is-street-food-safe-in-korea': ['???????????????????', '?????????????????????????????????????????????????????????????????????????????????????', '<p>??????????????????????????????????????????????????????????????</p>'],
      'is-wifi-free-in-korea': ['??????Wi-Fi???????', '??????????????????Wi-Fi??????????????????????????????????????????????????????????????eSIM?SIM???????????', '<p>???????????????????????????eSIM?10??20?35?????????????????????</p>'],
      'when-is-cherry-blossom-season-in-korea': ['??????????????', '3????4??????????????????????????3????????4??????????????1?????????????????????????????', '<p>?????????????????????????????????????????????????????????????????</p>'],
      'when-is-fall-foliage-in-korea': ['???????????????', '10????11???????????????????10????????????????10????????????????11??????????', '<p>??????????????????????????????????????????????????????</p>'],
      'what-is-closed-on-mondays-in-korea': ['??????????????', '?????????????????????????????????????????????????????????????????', '<p>????????????????????????????????????????????????????????????</p>'],
      'how-many-days-in-seoul': ['?????????????', '???????3?4????????????????????????????????????DMZ??????1?????KTX???????????', '<p>3??????????5?????????COEX??????????2?????????????????</p>'],
      'can-you-wear-shorts-in-korea': ['???????????????????', '????????????????????????????????????????????????????????????????????????????????????????????', '<p>?????????????????????????????????????????????????????????? ? ?????-10???????????</p>'],
      'is-korea-safe-at-night': ['????????????', '??????????????????????????????????????CCTV???????????????????????????????????????', '<p>???24?????????????????0???????????????????????????????????</p>'],
      'how-much-is-a-trip-to-korea': ['??????????????', '???????????1??55??????120???????250?????????1????????????800?1,000???????', '<p>??????9,000?13,000??????????????T-money????????????????????????????????</p>'],
    },
  },
  zh: {
    ui: { titleSuffix: '?2026?| KoreaPlus', badge: '????', moreH: '? ??????', ctaH: '??????????', ctaP: 'AI 30??????????', planBtn: '??? ??????', enBtn: '???? English' },
    items: {
      ...
      'best-time-to-visit-korea': ['????????????', '4?5??9?11???????????????????????????????7?8???????????????????????????', '<p>???3????????4????????????10????11?????????????7??????????</p>'],
      'do-they-speak-english-in-korea': ['???????????', '???????????????????????????????????????????????????????Papago?????????', '<p>?????????????????????????????????????????????????????????????????????</p>'],
      'do-you-tip-in-korea': ['??????????', '???????????????????????????????????????????????????????????????', '<p>??????????????????????????????????????gamsahamnida?????</p>'],
      'can-you-drink-tap-water-in-korea': ['??????????', '??????????????Arisu/??????????????????????????????????????????????????1,000???', '<p>??????????????????????????????????????????</p>'],
      'what-power-plug-does-korea-use': ['???????????', '???220V?60Hz????????????C??F????????????????????????????????????????220V???????', '<p>??????????100?240V??????????????????????Daiso??2,000?5,000???</p>'],
      'is-uber-available-in-korea': ['????Uber??', 'Uber??UT?????????????????????Kakao T?Kakao T????????????????????????????????', '<p>???????????????4,800?????????????????????</p>'],
      'can-you-use-google-maps-in-korea': ['??????????', '???????????????????????????????????????????????Naver???KakaoMap??????????????????', '<p>??????Naver???KakaoMap?????????????????????????????????????</p>'],
      'what-currency-does-korea-use': ['????????', '???????KRW?????????1,000???0.7?0.8????????????????5??10???????????????', '<p>????????????????????????????Global ATM?????</p>'],
      'do-i-need-cash-in-korea': ['??????????', '???????????????????????????????????????5??10??????????????????T-money???', '<p>??????????????????????????????????????????WOWPASS?T-money??????</p>'],
      'what-apps-do-i-need-for-korea': ['?????????App?', '?????Naver???KakaoMap????????????????Papago??????????Kakao T?????Korail Talk?KTX????????????????', '<p>?????KakaoMap???????????????App????????AI??????????????</p>'],
      'is-street-food-safe-in-korea': ['?????????????', '???????????????????????????????????????????????????????????????', '<p>??????????????????????????????????????????????</p>'],
      'is-wifi-free-in-korea': ['???WiFi????', '????????????????????WiFi?????????????????????????????????????????eSIM?SIM??', '<p>??????????????eSIM????20?35?????????????</p>'],
      'when-is-cherry-blossom-season-in-korea': ['???????????', '3????4???????????????3?????????4????????????????????????????', '<p>????????????????????????????????????????????????????????</p>'],
      'when-is-fall-foliage-in-korea': ['???????????', '10????11???????????10????????????????10??????????????11???', '<p>??????????????????????????????????????????????</p>'],
      'what-is-closed-on-mondays-in-korea': ['????????????', '???????????????????????????????????????????????????????', '<p>?????????????????????????????????????????????</p>'],
      'how-many-days-in-seoul': ['????????', '???????????????????????????????????????DMZ???????????KTX?????', '<p>3????????5????????/COEX?????????????</p>'],
      'can-you-wear-shorts-in-korea': ['?????????????', '?????????????????????????????????????????????????????????????', '<p>????????????????templestay????????????????????????????????-10?C????</p>'],
      'is-korea-safe-at-night': ['????????', '???????????????????????????????CCTV??????????????????????????????', '<p>???24????????????????????????????????????????????</p>'],
      'how-much-is-a-trip-to-korea': ['???????????', '??????????55??????120???????250?????????????800?1,000????????', '<p>??????9,000?13,000????????????T-money??????????????????????</p>'],
    },
  },
  es: {
    ui: { titleSuffix: ' (2026) | KoreaPlus', badge: 'Respuesta r?pida', moreH: '? M?s preguntas frecuentes', ctaH: '?Planeando un viaje a Corea?', ctaP: 'Crea un itinerario diario gratis con IA en segundos.', planBtn: '??? Crear mi itinerario gratis', enBtn: '???? English' },
    items: {
      'how-to-get-from-incheon-airport-to-seoul': ['?C?mo llego del aeropuerto de Incheon a Se?l?', 'Toma el tren AREX: el Expr?s llega a la estaci?n de Se?l en 43 minutos (11.000 ?) y el de todas las paradas en una hora aprox. (4.750 ? con T-money). Los autobuses limusina van directos a las zonas de hoteles, y el taxi cuesta 55.000?75.000 ? seg?n el tr?fico.', '<p>El AREX de todas las paradas es la mejor relaci?n calidad-precio y conecta con el metro en Hongik Univ., Gongdeok y la estaci?n de Se?l. Los autobuses limusina (17.000?18.000 ?) son ideales con equipaje voluminoso: paran en Myeongdong, Gangnam y la mayor?a de zonas hoteleras. Pasada la medianoche, busca los autobuses nocturnos N6001/N6703 o pide taxi con Kakao T.</p>'],
      'best-time-to-visit-korea': ['?Cu?l es la mejor ?poca para visitar Corea?', 'De abril a mayo y de septiembre a noviembre. La primavera trae cerezos en flor y clima suave; el oto?o, aire fresco y un follaje espectacular. El verano (julio?agosto) es caluroso, h?medo y lluvioso; el invierno es fr?o pero barato, tranquilo e ideal para esquiar.', '<p>Para los cerezos, apunta a finales de marzo (sur) o mediados de abril (Se?l). Para el follaje oto?al, de mediados de octubre a principios de noviembre es el punto ?lgido. Si puedes, evita las semanas del monz?n (jangma) de julio.</p>'],
      'do-they-speak-english-in-korea': ['?Se habla ingl?s en Corea?', 'Lo suficiente para viajar con comodidad: los carteles del metro, los men?s en zonas tur?sticas y los anuncios son biling?es, y los j?venes suelen entender ingl?s b?sico. Fuera de las zonas tur?sticas el ingl?s hablado es limitado, pero la app de traducci?n Papago cubre cualquier hueco.', '<p>Hoteles, aeropuertos y atracciones principales funcionan en ingl?s. En restaurantes locales, se?alar el men? m?s una app de traducci?n funciona perfectamente. Aprender dos frases ?????? (hola) y ????? (gracias)? gana simpat?a al instante.</p>'],
      'do-you-tip-in-korea': ['?Se da propina en Corea?', 'No. La propina no forma parte de la cultura coreana: ni en restaurantes, taxis, hoteles o peluquer?as. El precio que ves es el que pagas, e incluye impuestos. Dejar dinero extra incluso puede causar confusi?n.', '<p>El cargo por servicio en hoteles de lujo ya est? incluido en la cuenta. La mejor forma de mostrar agradecimiento es un sincero ????? (gamsahamnida, gracias).</p>'],
      'can-you-drink-tap-water-in-korea': ['?Se puede beber el agua del grifo en Corea?', 'S?: el agua del grifo coreana (llamada Arisu en Se?l) cumple las normas de la OMS y es segura. Aun as?, la mayor?a de los locales bebe agua filtrada o embotellada por costumbre. Una botella cuesta unos 1.000 ? en cualquier tienda de conveniencia.', '<p>Los restaurantes sirven agua filtrada gratis (a menudo t? de cebada) por defecto, y hay purificadores gratuitos en hoteles, centros comerciales y estaciones de metro para rellenar tu botella.</p>'],
      'what-power-plug-does-korea-use': ['?Qu? enchufe y voltaje se usa en Corea?', 'Corea usa 220V a 60Hz con enchufes redondos de dos clavijas tipo europeo (Tipo C y F). Quienes viajen desde EE. UU., Reino Unido, Jap?n y gran parte de Asia necesitan un adaptador; la mayor?a de los aparatos modernos admiten 220V sin transformador.', '<p>Mira si tu cargador indica ?100?240V?: si lo hace, solo necesitas un adaptador barato, a la venta en cualquier tienda de conveniencia o Daiso por 2.000?5.000 ?.</p>'],
      'is-uber-available-in-korea': ['?Hay Uber en Corea?', 'Uber opera en Corea como ?UT?, en alianza con taxis locales, pero todo el mundo usa Kakao T. Kakao T tiene interfaz en ingl?s, m?s conductores, tarifa con tax?metro y pago con tarjeta, lo que lo hace la mejor opci?n para visitantes.', '<p>Las tarifas est?n reguladas y son razonables: bajada de bandera de unos 4.800 ? en Se?l. Tambi?n puedes reservar taxis internacionales con conductores que hablan ingl?s en el aeropuerto.</p>'],
      'can-you-use-google-maps-in-korea': ['?Funciona Google Maps en Corea?', 'Solo en parte. Por la normativa sobre datos cartogr?ficos, Google Maps no da indicaciones en coche ni a pie en Corea, solo transporte p?blico. Usa Naver Map o KakaoMap: ambas tienen interfaz en ingl?s y datos locales mucho mejores.', '<p>Descarga Naver Map o KakaoMap antes de llegar y guarda tu hotel. Ambas muestran llegadas de autob?s en tiempo real, n?meros de salida del metro y mapas interiores de las estaciones principales.</p>'],
      'what-currency-does-korea-use': ['?Qu? moneda se usa en Corea?', 'Corea del Sur usa el won (KRW, ?). C?lculo r?pido: 1.000 ? ? 0,70?0,80 USD. Se acepta tarjeta casi en todas partes; lleva 50.000?100.000 ? en efectivo para mercados y comida callejera.', '<p>Usa nuestro conversor de divisas en tiempo real para ver la cotizaci?n de hoy, y saca wones en los cajeros ?Global ATM? de tiendas de conveniencia y bancos.</p>'],
      'do-i-need-cash-in-korea': ['?Necesito efectivo en Corea?', 'En general no: Corea es uno de los pa?ses m?s adaptados a la tarjeta del mundo, e incluso los puestos callejeros la aceptan cada vez m?s. Lleva 50.000?100.000 ? en efectivo para mercados tradicionales, peque?os comedores y recargar tu tarjeta T-money.', '<p>Las tarjetas extranjeras funcionan en la mayor?a de comercios, pero algunos pagos en webs locales exigen tarjeta coreana; ah? ayudan el pago en efectivo en tiendas de conveniencia o una tarjeta prepago (WOWPASS, T-money).</p>'],
      'what-apps-do-i-need-for-korea': ['?Qu? apps necesito para Corea?', 'Cuatro imprescindibles: Naver Map o KakaoMap (navegaci?n, Google Maps apenas funciona), Papago (el mejor traductor de coreano), Kakao T (taxis) y Korail Talk (billetes de tren KTX). Todas tienen interfaz en ingl?s y son gratuitas.', '<p>Extras ?tiles: las rese?as de restaurantes de KakaoMap, una app de comida a domicilio si tienes n?mero local, y nuestra propia Gu?a con IA para respuestas instant?neas sobre Corea.</p>'],
      'is-street-food-safe-in-korea': ['?Es segura la comida callejera en Corea?', 'S?: la comida callejera coreana es de las m?s seguras de Asia. Los puestos tienen licencia y se inspeccionan, la rotaci?n es r?pida (comida fresca) y millones la comen a diario. Los puestos concurridos y con mucha rotaci?n son la mejor apuesta.', '<p>Imprescindibles: tteokbokki, hotteok, eomuk (pastel de pescado) con caldo gratis y gimbap. El mercado de Gwangjang y los puestos nocturnos de Myeongdong son el punto de partida cl?sico.</p>'],
      'is-wifi-free-in-korea': ['?Es gratis el wifi en Corea?', 'En gran medida s?: Corea tiene uno de los wifis gratuitos m?s r?pidos y extendidos del mundo (metro, autobuses, caf?s, aeropuertos y plazas). Aun as?, lleva una eSIM o SIM para usar mapas y traducci?n sin cortes entre puntos de acceso.', '<p>Las redes gratuitas est?n por todas partes, pero una eSIM con datos ilimitados cuesta solo 20?35 USD por 10 d?as y elimina toda fricci?n.</p>'],
      'when-is-cherry-blossom-season-in-korea': ['?Cu?ndo es la temporada de cerezos en flor en Corea?', 'De finales de marzo a mediados de abril, avanzando de sur a norte: Jinhae y Busan florecen a finales de marzo y Se?l a principios de abril. La plena floraci?n dura solo una semana por ciudad, as? que consulta la previsi?n anual antes de reservar.', '<p>Mejores lugares: el Festival de Jinhae Gunhangje (el mayor de Corea), Yeouido y el lago Seokchon en Se?l, y el lago Bomun en Gyeongju. Nuestra p?gina de temporadas sigue la previsi?n de floraci?n en tiempo real cada primavera.</p>'],
      'when-is-fall-foliage-in-korea': ['?Cu?ndo es la temporada de follaje oto?al en Corea?', 'De mediados de octubre a principios de noviembre, avanzando de norte a sur: Seoraksan alcanza su punto ?lgido a mediados de octubre, Se?l (Bukhansan, palacios) a finales de octubre, y templos del sur como Naejangsan a principios de noviembre.', '<p>Naejangsan, Seoraksan y los parques de t?mulos de Gyeongju son los destinos de follaje ic?nicos. Comb?nalo con el Festival de la Danza de M?scaras de Andong o el Festival Internacional de Cine de Busan.</p>'],
      'what-is-closed-on-mondays-in-korea': ['?Qu? cierra los lunes en Corea?', 'La mayor?a de los museos (incluido el Museo Nacional de Corea) cierran los lunes, y el palacio Changdeokgung tambi?n. Gyeongbokgung cierra los martes en su lugar. Mercados, calles comerciales y templos abren a diario.', '<p>Organiza los palacios seg?n la divisi?n: Gyeongbokgung el lunes, Changdeokgung cualquier d?a menos el lunes. Si un festivo cae en lunes, muchos museos cierran el martes.</p>'],
      'how-many-days-in-seoul': ['?Cu?ntos d?as se necesitan en Se?l?', 'Tres o cuatro d?as cubren lo esencial de Se?l: palacios y aldeas hanok, Myeongdong y Hongdae, una tarde en el r?o Han y una excursi?n (DMZ o la isla Nami). Una semana permite a?adir Busan en KTX.', '<p>Con 3 d?as, sigue nuestra ruta cl?sica; con 5 o m?s, a?ade Gangnam/COEX, m?s barrios y una segunda excursi?n.</p>'],
      'can-you-wear-shorts-in-korea': ['?Se pueden llevar pantalones cortos en Corea? (C?digo de vestimenta)', 'S?: pantalones cortos, faldas y ropa informal son totalmente normales en las ciudades coreanas, sobre todo en el h?medo verano. Los ?nicos lugares que requieren vestir con recato son templos y ceremonias reales: cubre hombros y rodillas all?.', '<p>Los coreanos visten con estilo pero informales. Para las estancias en templos se proporciona o se exige ropa holgada y discreta. En invierno, prioriza las capas de abrigo: la moda nunca gana a una sensaci?n t?rmica de -10 ?C.</p>'],
      'is-korea-safe-at-night': ['?Es seguro Corea de noche?', 'S?: Corea es uno de los pa?ses m?s seguros del mundo de noche. Las calles siguen concurridas y bien iluminadas pasada la medianoche, hay mucha cobertura de c?maras y los delitos violentos contra visitantes son rar?simos, tambi?n para mujeres que viajan solas.', '<p>Los mercados nocturnos, los caf?s 24 horas y el metro hasta cerca de medianoche son parte de la vida diaria. Basta con la prudencia habitual de cualquier gran ciudad en las zonas de ocio nocturno.</p>'],
      'how-much-is-a-trip-to-korea': ['?Cu?nto cuesta un viaje a Corea?', 'Sin contar los vuelos, un viajero econ?mico gasta unos 55 USD/d?a, uno de gama media 120 USD/d?a y uno de confort 250 USD/d?a. Un viaje t?pico de una semana de gama media ronda los 800?1.000 USD m?s el billete de avi?n.', '<p>La comida es la ganga: comidas excelentes por 9.000?13.000 ?. El transporte es barato con T-money. Mira el desglose completo con presupuestos de ejemplo en nuestro blog.</p>'],
    },
  },
};
...
const THEMES = [
  ...
  { slug: 'korea-food-tour-5-day-itinerary', emoji: '??', days: 5, h1: '5-Day Korea Food Itinerary (Eat Like a Local)',
    ...
    tips: ['Come hungry to Gwangjang Market ? share portions so you can try more stalls.', 'Lunch sets (??) at office-worker restaurants are the best value in Korea.', 'Day-trip to Jeonju by KTX for the definitive bibimbap and choco pie.', 'Convenience stores are legit: build one breakfast entirely from GS25.'] },
  ...
  { slug: 'winter-korea-5-day-itinerary', emoji: '??', days: 5, h1: '5-Day Winter Korea Itinerary (Snow, Ski & Spas)',
...
function buildTheme(t) {
  ...
  body += keyFactsBox([`?? ${t.days}-day route`, `?? Base: ${esc((THEME_CITY[t.slug] || 'Seoul'))}`, t.month ? `??? Best: ${t.month[0].toUpperCase() + t.month.slice(1)}` : `??? Best: Apr?May & Sep?Nov`, `?? Themed route`]);
  ...
  for (let d = 1; d <= t.days; d++) {
    ...
    if (m) body += slotHtml('??', 'Morning', m);
    if (l) body += slotHtml('??', 'Lunch', l);
    if (a && (!m || a.slug !== m.slug)) body += slotHtml('???', 'Afternoon', a);
  ...
  body += `<h2>??? More Korea itineraries</h2><div class="seo-linklist">${themeRel.map(x => `<a href="itinerary/${x.slug}.html">${x.emoji} ${esc(x.h1.split('(')[0].trim())}</a>`).join('')}<a href="itinerary/seoul-3-day-itinerary.html">??? 3-Day Seoul Itinerary</a></div>`;
  const qa = [
...
const ITIN_UI = {
  ja: { tipsH: '?? ?????', faqH: '? ??????', ctaH: '???????????', ctaP: 'AI??????????????????????', planBtn: '??? ????????', enBtn: '???? English' },
  zh: { tipsH: '?? ????', faqH: '? ????', ctaH: '????????', ctaP: 'AI ??????????????', planBtn: '??? ??????', enBtn: '???? English' },
  es: { tipsH: '?? Consejos', faqH: '? Preguntas frecuentes', ctaH: 'Personaliza este itinerario', ctaP: 'La IA lo rehace seg?n tus fechas, gratis.', planBtn: '??? Crear itinerario gratis', enBtn: '???? English' },
  ko: { tipsH: '?? ?? ?', faqH: '? ?? ?? ??', ctaH: '? ??? ? ????', ctaP: 'AI? ??? ?? ??? ?? ? ???.', planBtn: '??? ??? ?? ???', enBtn: '???? English' },
  fr: { tipsH: '?? Conseils', faqH: '? Questions fr?quentes', ctaH: 'Personnalise cet itin?raire', ctaP: 'L\'IA le refait gratuitement selon tes dates.', planBtn: '??? Cr?er mon itin?raire gratuit', enBtn: '???? English' },
  de: { tipsH: '?? Reisetipps', faqH: '? H?ufige Fragen', ctaH: 'Pass diesen Reiseplan an', ctaP: 'Die KI erstellt ihn gratis nach deinen Daten neu.', planBtn: '??? Gratis Reiseplan erstellen', enBtn: '???? English' },
  pt: { tipsH: '?? Dicas', faqH: '? Perguntas frequentes', ctaH: 'Personalize este roteiro', ctaP: 'A IA refaz conforme suas datas, de gra?a.', planBtn: '??? Criar roteiro gr?tis', enBtn: '???? English' },
  id: { tipsH: '?? Tips', faqH: '? Pertanyaan umum', ctaH: 'Sesuaikan itinerary ini', ctaP: 'AI menyusun ulang sesuai tanggalmu, gratis.', planBtn: '??? Buat itinerary gratis', enBtn: '???? English' },
};
...
function buildItineraryL10n(itSlug, lang) {
  ...
  const emoji = (THEMES.find(x => x.slug === itSlug) || {}).emoji || '???';
  ...
  if (qa.length) body += `<h2>${esc(ui.faqH)}</h2><div class="seo-faq">${qa.map(([q, a]) => `<details><summary>${esc(q)}</summary><p>${esc(a)}</p></details>`).join('')}</div>`;
  body += `<div class="seo-cta"><h2>${esc(ui.ctaH)}</h2><p>${esc(ui.ctaP)}</p><div class="btns"><a class="primary" href="plan.html">${esc(ui.planBtn)}</a><a class="ghost" href="${enUrl.replace(BASEP, '')}">${esc(ui.enBtn)}</a></div></div>`;
  const hero = `<header class="seo-hero"><span class="emoji">${emoji}</span><h1>${esc(t.h1)}</h1><div class="meta"><span class="seo-badge">${(t.days || []).length} ${lang === 'ja' ? '??' : lang === 'zh' ? '?' : 'd?as'}</span></div></header>`;
...
const COMPARES = [
  { slug: 'seoul-vs-busan', emoji: '??', h1: 'Seoul vs Busan: Which Should You Visit?',
  ...
  { slug: 'jeju-vs-busan', emoji: '???', h1: 'Jeju vs Busan: Which Coastal Escape?',
...
function buildCityFood(city) {
  ...
  body += `<h2>?? Eating tips for ${esc(n)}</h2><ul class="tips"><li>Side dishes (banchan) are free and refillable ? ask with "? ???".</li><li>Lunch set menus give the best value; save dinner for BBQ and stews.</li><li>Markets peak in the evening from 5pm; bring some cash.</li></ul>`;
...
const SEASONS4 = [
  ...
  { slug: 'summer', name: 'Summer', emoji: '???', mi: [5, 6, 7], itin: 'first-time-korea-7-day-itinerary', hl: 'beaches, mud festivals and electric Han River nights' },
  ...
  { slug: 'winter', name: 'Winter', emoji: '??', mi: [11, 0, 1], itin: 'winter-korea-5-day-itinerary', hl: 'snowy palaces, ski resorts and steaming street food' },
];
...
const KPOP_HIST_EMOJI = { 'kpop-history': '??', 'first-generation-kpop': '1??', 'second-generation-kpop': '2??', 'third-generation-kpop': '3??', 'fourth-generation-kpop': '4??', 'fifth-generation-kpop': '5??', 'sm-entertainment-history': '???', 'yg-entertainment-history': '??', 'jyp-entertainment-history': '??', 'hybe-history': '??' };
const KPOP_HIST_MORE = { en: '?? More K-Pop history', ko: '?? K-pop ?? ???', ja: '?? K-POP?????????', zh: '?? ?? K-pop ??', es: '?? M?s historia del K-pop', fr: '?? Plus d\'histoire de la K-pop', de: '?? Mehr K-Pop-Geschichte', pt: '?? Mais hist?ria do K-pop', id: '?? Sejarah K-pop lainnya', ar: '?? ?????? ?? ????? K-Pop', hi: '?? K-Pop ?????? ?? ?? ????', ru: '?? ??? ?? ??????? K-pop', vi: '?? Th?m v? l?ch s? K-pop', th: '?? ??????? K-pop ?????????' };
...
function buildKpopHistory(slug, lang) {
  ...
  const url = kpopPublicUrl(lang, slug);
  const enUrl = kpopPublicUrl('en', slug);
  ...
  body += `<h2>${esc(KPOP_HIST_MORE[lang] || KPOP_HIST_MORE.en)}</h2><div class="seo-linklist">${sibs.map(s2 => `<a href="${kpopPublicUrl(lang, s2)}">${KPOP_HIST_EMOJI[s2] || '??'} ${esc(KPOP_HIST[s2][lang].h1)}</a>`).join('')}<a href="${kpopPublicUrl(lang, 'browse')}">${esc(browseLabel)}</a><a href="/kpop">?? K-Pop Hub</a></div>`;
  body += affBlock({ city: 'Seoul', cat: 'general', q: '', lang });
  ...
  const alts = lang === 'en'
    ? others.map(l => ({ lang: l, url: kpopPublicUrl(l, slug) }))
    : [{ lang: 'en', url: enUrl }, ...others.filter(l => l !== 'en').map(l => ({ lang: l, url: kpopPublicUrl(l, slug) }))];
...
const KPOP_CAT = {
  artists:   { emoji: '??', label: { en: 'Artists & Groups', ko: '???? ? ??', ja: '???????????', zh: '?? ? ??', es: 'Artistas y grupos', fr: 'Artistes et groupes', de: 'K?nstler & Gruppen', pt: 'Artistas e grupos', id: 'Artis & Grup', ar: '???????? ??????', hi: '???????? ?? ?????', ru: '??????? ? ??????', vi: 'Ngh? s? v? nh?m nh?c', th: '???????????' } },
  ...
  members:   { emoji: '??', label: { en: 'Idols by Group', ko: '??? ???', ja: '?????????', zh: '??????', es: '?dolos por grupo', fr: 'Idols par groupe', de: 'Idols nach Gruppe', pt: '?dolos por grupo', id: 'Idol per grup', ar: '?????? ??? ??????', hi: '????? ?? ????? ?? ????', ru: '?????? ?? ???????', vi: 'Idol theo nh?m', th: '?????????????' } },
  birthdays: { emoji: '??', label: { en: 'Birthdays & Zodiac', ko: '?? ? ??? ? ?', ja: '?????????', zh: '?? ? ?? ? ??', es: 'Cumplea?os y zodiaco', fr: 'Anniversaires et zodiaque', de: 'Geburtstage & Sternzeichen', pt: 'Anivers?rios e zod?aco', id: 'Ulang tahun & zodiak', ar: '????? ??????? ????????', hi: '??????? ?? ????', ru: '??? ???????? ? ????? ???????', vi: 'Ng?y sinh v? cung ho?ng ??o', th: '??????????????' } },
  lightsticks: { emoji: '??', label: { en: 'Lightsticks', ko: '???', ja: '?????', zh: '???', es: 'Lightsticks', fr: 'Lightsticks', de: 'Lightsticks', pt: 'Lightsticks', id: 'Lightstick', ar: '????? ???????', hi: '?????????', ru: '?????????', vi: 'Lightstick', th: '??????' } },
  compare:   { emoji: '??', label: { en: 'Group Comparisons', ko: '?? ??', ja: '??????', zh: '????', es: 'Comparativas de grupos', fr: 'Comparatifs de groupes', de: 'Gruppenvergleiche', pt: 'Compara??es de grupos', id: 'Perbandingan grup', ar: '??????? ??? ?????', hi: '????? ?? ????-????? ?????', ru: '????????? ?????', vi: 'So s?nh nh?m', th: '????????????' } },
  releases:  { emoji: '??', label: { en: 'Releases by Year', ko: '??? ??', ja: '??????', zh: '??????', es: 'Lanzamientos por a?o', fr: 'Sorties par ann?e', de: 'Releases nach Jahr', pt: 'Lan?amentos por ano', id: 'Rilis per tahun', ar: '????????? ??? ?????', hi: '??? ?? ????? ?? ??????', ru: '?????? ?? ?????', vi: 'Nh?c ph?t h?nh theo n?m', th: '?????????????' } },
  basics:    { emoji: '??', label: { en: 'K-Pop Basics & Terms', ko: 'K-pop ?? ? ??', ja: 'K-POP?????', zh: 'K-pop ?? ? ??', es: 'Conceptos y t?rminos', fr: 'Bases et termes', de: 'Grundlagen & Begriffe', pt: 'B?sico e termos', id: 'Dasar & Istilah', ar: '??????? K-Pop ?????????', hi: 'K-Pop ??????? ?? ????????', ru: '?????? ? ??????? K-pop', vi: 'Nh?p m?n v? thu?t ng? K-pop', th: '??????????????? K-pop' } },
  legends:   { emoji: '?', label: { en: 'Legendary Acts', ko: '??? ????', ja: '?????', zh: '????', es: 'Grupos legendarios', fr: 'Groupes l?gendaires', de: 'Legend?re Acts', pt: 'Grupos lend?rios', id: 'Artis Legendaris', ar: '??????? ?????????', hi: '???????? ????????', ru: '??????????? ???????', vi: 'Ngh? s? huy?n tho?i', th: '????????????????' } },
  history:   { emoji: '???', label: { en: 'History & Generations', ko: '?? ? ??', ja: '?????', zh: '?? ? ??', es: 'Historia y generaciones', fr: 'Histoire et g?n?rations', de: 'Geschichte & Generationen', pt: 'Hist?ria e gera??es', id: 'Sejarah & Generasi', ar: '??????? ????????', hi: '?????? ?? ???????', ru: '??????? ? ?????????', vi: 'L?ch s? v? c?c th? h?', th: '????????????????????' } },
  companies: { emoji: '??', label: { en: 'Agencies', ko: '???', ja: '???', zh: '????', es: 'Agencias', fr: 'Agences', de: 'Agenturen', pt: 'Ag?ncias', id: 'Agensi', ar: '????? ???????', hi: '?????????', ru: '?????????', vi: 'C?ng ty ch? qu?n', th: '????????' } },
  global:    { emoji: '??', label: { en: 'Global Records & Culture', ko: '??? ? ?? ? ??', ja: '??????????', zh: '???? ? ??', es: 'R?cords globales y cultura', fr: 'Records mondiaux et culture', de: 'Globale Rekorde & Kultur', pt: 'Recordes globais e cultura', id: 'Rekor Global & Budaya', ar: '??????? ???????? ???????? ????????', hi: '?????? ??????? ?? ????????', ru: '??????? ??????? ? ????????', vi: 'K? l?c to?n c?u v? v?n h?a', th: '????????????????????????' } },
};
...
const KPOP_BROWSE_T = {
  ...
  ko: { h1: 'K-pop ?? ????', title: 'K-pop ?? ???? ? ???? ? ?? ? ??? | KoreaPlus', lead: 'KoreaPlus? ?? K-pop ???? ????? ?????? ? ???? ???, ? ??, ??, ???, ?? ????. ????? ?? ?? ??? ???.', girl: '???', boy: '????', solo: '??', more: '?? ?? ????' },
  ja: { h1: 'K-POP??????', title: 'K-POP????? ? ???????????? | KoreaPlus', lead: 'KoreaPlus?K-POP???????????????????????????????????????????????????????????', girl: '???????', boy: '???????', solo: '??', more: '?? ?????' },
  zh: { h1: '???? K-pop', title: 'K-pop ???? ? ?? ? ?? ? ?? | KoreaPlus', lead: 'KoreaPlus ??? K-pop ???????? ?? ?????????????????????????????????', girl: '??', boy: '??', solo: 'solo ??', more: '?? ????' },
  es: { h1: 'Explora todo el K-pop', title: 'Todo el K-pop ? Artistas, historia y gu?as | KoreaPlus', lead: 'Todas las gu?as de K-pop de KoreaPlus, ordenadas por categor?as: perfiles, t?rminos de fans, historia, agencias y r?cords globales. Elige una categor?a y entra.', girl: 'Grupos femeninos', boy: 'Grupos masculinos', solo: 'Solistas', more: '?? Explorar todo' },
  fr: { h1: 'Explorer tout le K-pop', title: 'Tout le K-pop ? Artistes, histoire et guides | KoreaPlus', lead: 'Tous les guides K-pop de KoreaPlus, class?s par cat?gories : profils, termes de fans, histoire, agences et records mondiaux. Choisissez une cat?gorie et explorez.', girl: 'Girl groups', boy: 'Boy groups', solo: 'En solo', more: '?? Tout explorer' },
  de: { h1: 'Alle K-Pop entdecken', title: 'Alles ?ber K-Pop ? K?nstler, Geschichte & Guides | KoreaPlus', lead: 'Alle K-Pop-Guides von KoreaPlus, nach Kategorien sortiert: Profile, Fan-Begriffe, Geschichte, Agenturen und Weltrekorde. Kategorie w?hlen und eintauchen.', girl: 'Girlgroups', boy: 'Boygroups', solo: 'Solok?nstler', more: '?? Alle entdecken' },
  pt: { h1: 'Explorar todo o K-pop', title: 'Todo o K-pop ? Artistas, hist?ria e guias | KoreaPlus', lead: 'Todos os guias de K-pop do KoreaPlus, organizados por categorias: perfis, termos de f?s, hist?ria, ag?ncias e recordes globais. Escolha uma categoria e explore.', girl: 'Girl groups', boy: 'Boy groups', solo: 'Solistas', more: '?? Explorar tudo' },
  ...
  ar: { h1: '?????? ?? ?? ??? K-Pop', title: '?? ?? ??? K-Pop ? ???????? ???????? ??????? | KoreaPlus', lead: '?? ???? K-Pop ??? KoreaPlus? ???????? ??? ?????: ????? ???????? ???????? ???????? ???????? ?????? ??????? ???????? ???????? ????????. ???? ??? ????? ?????????.', girl: '??? ???????', boy: '??? ???????', solo: '???????? ?????????', more: '?? ?????? ????' },
  hi: { h1: '???? K-Pop ????????? ????', title: '???? K-Pop ? ????????, ?????? ?? ???? | KoreaPlus', lead: 'KoreaPlus ?? ?? K-Pop ????, ??????? ?? ????? ?? ?? ??? ? ???????? ????????, ????? ?? ????, ??????, ????????? ?? ?????? ???????? ??? ??????? ????? ?? ????????? ???? ???? ?????', girl: '???? ?????', boy: '??? ?????', solo: '???? ????????', more: '?? ??? K-Pop ????' },
  ru: { h1: '???? K-pop', title: '???? K-pop ? ???????, ??????? ? ???? | KoreaPlus', lead: '??? ???? ?? K-pop ?? KoreaPlus, ??????????? ?? ??????????: ??????? ????????, ??????? ???????, ???????, ????????? ? ??????? ???????. ???????? ????????? ? ????????????.', girl: '????-??????', boy: '???-??????', solo: '??????? ???????', more: '?? ???? K-pop' },
  vi: { h1: 'Kh?m ph? to?n b? K-pop', title: 'To?n b? K-pop ? Ngh? s?, l?ch s? v? c?m nang | KoreaPlus', lead: 'T?t c? c?m nang K-pop tr?n KoreaPlus, x?p theo t?ng nh?m ch? ?? ? h? s? ngh? s?, thu?t ng? fandom, l?ch s?, c?ng ty ch? qu?n v? k? l?c to?n c?u. Ch?n m?t nh?m ch? ?? r?i ?i s?u v?o b?n trong.', girl: 'Nh?m n?', boy: 'Nh?m nam', solo: 'Ngh? s? solo', more: '?? Xem to?n b?' },
  th: { h1: '????? K-pop ???????', title: 'K-pop ??????? ? ?????? ??????? ????????? | KoreaPlus', lead: '?????? K-pop ??????????? KoreaPlus ?????????????? ? ????????????? ???????????? ??????? ???????? ???????????????? ????????????????????????????????????', girl: '???????????', boy: '????????', solo: '????????????', more: '?? ?????????' },
};
...
function buildKpopBrowse(lang) {
  ...
  const dir = KPOP_HIST_DIR(lang);
  const url = kpopPublicUrl(lang, 'browse');
  ...
  const link = (slug) => {
    ...
    return `<a href="${kpopPublicUrl(lang, slug)}">${emoji || (/-profile$/.test(slug) ? '??' : '??')} ${esc(KPOP_HIST[slug][lang].h1)}</a>`;
  };
  ...
  const alts = lang === 'en'
    ? others.map(l => ({ lang: l, url: kpopPublicUrl(l, 'browse') }))
    : [...(kpopHasPages('en') ? [{ lang: 'en', url: kpopPublicUrl('en', 'browse') }] : []), ...others.filter(l => l !== 'en').map(l => ({ lang: l, url: kpopPublicUrl(l, 'browse') }))];
...
const CTX = {
  ...
  kpopPublicUrl, kpopVsPublicUrl, kpopFileRel, kpopLangDir,
...
// === GENERATED CONTENT MODULES (require/out ?? ??) ===
...
## Languages
- English (primary), ??? (/ja/), ?? (/zh/), Espa?ol (/es/) for seasonal & visa guides.
`;
